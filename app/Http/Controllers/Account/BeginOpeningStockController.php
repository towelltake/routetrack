...
class BeginOpeningStockController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        if ($selectedRoute > 0) {
            abort_unless($scope->allows($user, 'route', $selectedRoute), 403);
        }
        ...
        if (!Schema::hasTable('inventorytransactionheader') || !Schema::hasTable('inventorytransactiondetail')) {
            return Inertia::render('account/transaction/begin-opening-stock/Index', [
        ...
        $query = DB::table('inventorytransactionheader as header')
            ...
            ->selectRaw('
                ...
                COALESCE(header.documentnumber, "") as documentnumber,
            ...
            ->whereExists(function ($subQuery) {
                ...
                if (Schema::hasColumn('inventorytransactiondetail', 'transactiontypecode')) {
                    $subQuery->where('detail.transactiontypecode', 12);
                }
            });
    ...
    public function show(Request $request, int $detailkey): Response
    {
        ...
        $header = DB::table('inventorytransactionheader as header')
            ...
            ->selectRaw('
                ...
                ' . $this->transactionTimeExpression('header') . ' as transactiontime,
                ' . $this->columnExpression('inventorytransactionheader', 'routestartdate', 'header.routestartdate') . ' as routestartdate,
                ' . $this->columnExpression('inventorytransactionheader', 'accperiod', 'header.accperiod') . ' as accperiod,
                ' . $this->columnExpression('inventorytransactionheader', 'voidflag', 'header.voidflag') . ' as voidflag
            ')
            ->first();
        ...
        $lineQuery = DB::table('inventorytransactiondetail as detail')
            ...
            ->selectRaw('
                detail.itemcode,
                COALESCE(item.alternatecode, "") as alternatecode,
                COALESCE(item.itemshortdescription, "") as itemshortdescription,
                COALESCE(item.unitspercase, 1) as unitspercase,
        ...
        if (Schema::hasColumn('inventorytransactiondetail', 'transactiontypecode')) {
            $lineQuery->where('detail.transactiontypecode', 12);
        }
        ...
        return Inertia::render('account/transaction/begin-opening-stock/Show', [
            'header' => [
    ...
    {
        $query = app(AccessScopeService::class)->scopeQuery(request()->user(), DB::table('routemaster'), 'route', 'routecode')
