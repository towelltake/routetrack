...
class CustomerController extends Controller
{
    public function index(): Response
    {
        ...
        $customers = CustomerMaster::query()
            ->where('customermaster.templateindicator', 0)
            ->leftJoin('routemaster as route', 'route.routecode', '=', 'customermaster.routecode')
            ->leftJoin('channelmaster', 'channelmaster.channelcode', '=', 'customermaster.channelcode')
            ->leftJoin('categorymaster', 'categorymaster.categoryid', '=', 'customermaster.customercategory')
            ->when($search, function ($query, $searchTerm) {
                $query->where(function ($inner) use ($searchTerm) {
                    $inner->where('customermaster.customercode', 'like', '%' . $searchTerm . '%')
                        ...
                        ->orWhere('customermaster.arbcustomername', 'like', '%' . $searchTerm . '%')
                        ->orWhere('route.routename', 'like', '%' . $searchTerm . '%')
                        ->orWhere('channelmaster.channelname', 'like', '%' . $searchTerm . '%')
                        ->orWhere('categorymaster.categoryname', 'like', '%' . $searchTerm . '%');
                });
            })
            ...
            ->select([
                ...
                'channelmaster.channelname as channelname',
                'categorymaster.categoryname as categoryname',
            ])
            ->paginate($perPage)
            ->withQueryString();

        return Inertia::render('account/customer/Index', [
            'filters' => [
                'search' => $search,
                'per_page' => $perPage,
            ],
    ...
    public function create(): Response
    {
        ...
        return Inertia::render('account/customer/Create', $props);
    }

    public function show(CustomerMaster $customer): Response
    {
        abort_unless(app(AccessScopeService::class)->allows(request()->user(), 'route', $customer->routecode), 403);

        return Inertia::render('account/customer/View', $this->formProps($customer));
    }

    public function edit(CustomerMaster $customer): Response
    {
        abort_unless(app(AccessScopeService::class)->allows(request()->user(), 'route', $customer->routecode), 403);

        return Inertia::render('account/customer/Edit', $this->formProps($customer));
    }
    ...
    public function update(Request $request, CustomerMaster $customer): RedirectResponse
    {
        abort_unless(app(AccessScopeService::class)->allows($request->user(), 'route', $customer->routecode), 403);

        $payload = $this->validatedData($request, $customer);
    ...
    public function destroy(CustomerMaster $customer): RedirectResponse
    {
        abort_unless(app(AccessScopeService::class)->allows(request()->user(), 'route', $customer->routecode), 403);
    ...
    public function downloadBulkImportTemplate(): HttpResponse
    {
        return ExcelXmlWorkbook::download(
            'account-customer-bulk-import-template.xls',
    ...
    public function bulkImport(Request $request): RedirectResponse
    {
        ...
        return redirect()
            ->route('account.customer.index')
            ->with('success', $imported . ' customer record(s) imported successfully.');
    }
    ...
    private function customerFormData(?CustomerMaster $customer): array
    {
        ...
        $data = array_merge($this->defaultCustomerData(), array_intersect_key(
            $record,
            array_flip(array_keys($this->defaultCustomerData()))
        ));

        foreach (['delivery_slot_from', 'delivery_slot_to'] as $field) {
    ...
    private function defaultCustomerData(): array
    {
        return [
            ...
            'barcode' => '',
            'customerphone' => '',
            'delivery_slot_from' => null,
            'delivery_slot_to' => null,
            'autojp_priority' => 0,
            'allow_cross_route_jp' => 0,
            'customeraddress1' => '',
            'customeraddress2' => '',
            'customeraddress3' => '',
            'customercity' => '',
    ...
    {
        return $this->validatePayload($request->all(), $customer);
    }

    private function validatePayload(array $data, ?CustomerMaster $customer = null): array
    {
        $data = $this->normalizeLegacyOptionDefaults($data);

        $data = Validator::make($data, [
            'routecode' => ['required', 'integer', Rule::exists('routemaster', 'routecode')],
            'headofficecode' => [
                Rule::requiredIf(($data['customertype'] ?? null) === '2'),
                'nullable',
                'integer',
                Rule::exists('customermaster', 'customercode'),
            ],
            ...
            'delivery_slot_from' => ['nullable', 'date_format:H:i'],
            'delivery_slot_to' => ['nullable', 'date_format:H:i', 'after:delivery_slot_from'],
            'autojp_priority' => ['nullable', 'integer', 'min:0', 'max:100'],
            'allow_cross_route_jp' => ['nullable', 'integer', Rule::in([0, 1])],
            ...
            'customertype' => ['required', Rule::in(['1', '2', '3'])],
            ...
            'pricingkey' => $this->pricingKeyRules(),
            'promotionkey' => $this->promotionKeyRules(),
            ...
            'activecustomer' => ['nullable', 'integer', Rule::in([0, 1])],
            'arcustomertype' => ['nullable', 'integer', Rule::in([0, 1])],
        ...
        foreach ([
            ...
            'delivery_slot_from', 'delivery_slot_to',
            'customeraddress3', 'customercity', 'customerzip', 'pobox', 'channelcode', 'customercategory', 'currencycode',
            'pricingkey', 'promotionkey', 'creditlimit', 'creditlimitdays', 'tclimit', 'messagekey1', 'messagekey2', 'messagekey3', 'messagekey4',
    ...
    private function mapBulkImportRow(array $row): array
    {
        ...
        return array_replace($this->defaultCustomerData(), [
            'routecode' => $this->integerOrNull($row['route_code'] ?? null),
            'headofficecode' => $this->integerOrNull($row['head_office_code'] ?? null),
            'customername' => $this->nullIfBlank($row['customer_name'] ?? null),
            'arbcustomername' => $this->nullIfBlank($row['arabic_name'] ?? null),
            'alternatecode' => $this->nullIfBlank($row['alternate_code'] ?? null),
            'barcode' => $this->nullIfBlank($row['barcode'] ?? null),
            'customerphone' => $this->nullIfBlank($row['phone'] ?? null),
            'delivery_slot_from' => $this->normalizeImportedTime($row['delivery_slot_from'] ?? null),
            'delivery_slot_to' => $this->normalizeImportedTime($row['delivery_slot_to'] ?? null),
            'autojp_priority' => $this->integerOrDefault($row['autojp_priority'] ?? null, 0),
            'allow_cross_route_jp' => $this->normalizeFlag($row['allow_cross_route_jp'] ?? null),
            'customeraddress1' => $this->nullIfBlank($row['address_1'] ?? null),
            'customeraddress2' => $this->nullIfBlank($row['address_2'] ?? null),
            'customeraddress3' => $this->nullIfBlank($row['address_3'] ?? null),
            'customercity' => $this->nullIfBlank($row['city'] ?? null),
            'customerzip' => $this->nullIfBlank($row['zip'] ?? null),
            'pobox' => $this->nullIfBlank($row['po_box'] ?? null),
            'customertype' => (string) ($this->nullIfBlank($row['customer_type'] ?? null) ?? '1'),
            'channelcode' => $this->integerOrNull($row['channel_code'] ?? null),
            'customercategory' => $this->integerOrNull($row['category_code'] ?? null),
            'currencycode' => $this->integerOrNull($row['currency_code'] ?? null),
            'invoicepaymentterms' => $this->integerOrDefault($row['payment_terms'] ?? null, 0),
            'creditlimit' => $this->decimalOrNull($row['credit_limit'] ?? null),
            'creditlimitdays' => $this->integerOrNull($row['credit_limit_days'] ?? null),
            'activecustomer' => $this->normalizeFlag($row['status'] ?? null),
            'arcustomertype' => $this->normalizeFlag($row['ar_customer_type'] ?? null),
        ]);
    }
    ...
    private function decimalOrNull(mixed $value): int|float|null
    {
        $value = $this->nullIfBlank($value);
    ...
    private function pricingKeyOptions()
    {
        if (! Schema::hasTable('customerpricingplanheader1')) {
            return collect();
        }

        $query = CustomerPricingPlanHeader1::query()->orderBy('pricingplankey');

        if (Schema::hasColumn('customerpricingplanheader1', 'activeindicator')) {
            $query->where('activeindicator', 1);
        }

        return $query->get(['pricingplankey as id', DB::raw("CONCAT(pricingplankey, ' -- ', COALESCE(description, '')) as label")]);
    }

    private function promotionKeyOptions()
    {
        if (! Schema::hasTable('promokeyheader')) {
            return collect();
        }

        $query = PromoKeyHeader::query()->orderBy('promotionkey');

        if (Schema::hasColumn('promokeyheader', 'activeindicator')) {
            $query->where('activeindicator', 1);
        }

        return $query->get(['promotionkey as id', DB::raw("CONCAT(promotionkey, ' -- ', COALESCE(description, '')) as label")]);
    }
    ...
    private function pricingKeyRules(): array
    {
        $rules = ['nullable', 'integer'];

        if (Schema::hasTable('customerpricingplanheader1')) {
            $rules[] = Rule::exists('customerpricingplanheader1', 'pricingplankey');
        }

        return $rules;
    }

    private function promotionKeyRules(): array
    {
        $rules = ['nullable', 'integer'];

        if (Schema::hasTable('promokeyheader')) {
            $rules[] = Rule::exists('promokeyheader', 'promotionkey');
        }
