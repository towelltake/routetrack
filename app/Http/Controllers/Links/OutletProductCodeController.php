...
class OutletProductCodeController extends Controller
{
    ...
    {
        return Inertia::render('links/outlet-product-code/Index', [
            'available' => $this->hasRequiredTables(),
            'formMeta' => [
                'title' => 'Outlet Product Code',
                'subtitle' => 'Assign an outlet product code to customers by selected region, depot, area, or route',
                'indexUrl' => '/links/outlet-product-code',
                'loadUrl' => '/links/outlet-product-code/load',
                'saveUrl' => '/links/outlet-product-code/save',
                'permission' => 'outlet product code',
            ],
            'optionSets' => [
                'outletProductCodeOptions' => $this->outletProductCodeOptions(),
    ...
    public function load(Request $request): JsonResponse
    {
        ...
        $selectedCustomers = $this->customerQuery(
            ...
            ->where('cust.outletsubtype', (int) $data['outlet_product_code'])
            ->get();
    ...
    public function save(Request $request): RedirectResponse
    {
        ...
        $outletProductCode = (int) $data['outlet_product_code'];
        ...
        DB::transaction(function () use ($outletProductCode, $fillBy, $filterValue, $categoryId, $customerIds) {
            // Preserve the legacy NMWC workflow: clear outlet subtype only inside the selected filter scope.
            $this->outletScopedQuery($fillBy, $filterValue, $categoryId)
                ->where('cust.outletsubtype', $outletProductCode)
                ->update(['outletsubtype' => null]);

            if ($customerIds->isNotEmpty()) {
                DB::table('customermaster')
                    ->whereIn('customercode', $customerIds->all())
                    ->update(['outletsubtype' => $outletProductCode]);
            }
        });

        return redirect('/links/outlet-product-code')->with('success', 'Update Record');
    }

    private function validatedPayload(Request $request, bool $withCustomers): array
    {
        $rules = [
            'outlet_product_code' => ['required', 'integer', Rule::exists('groupmaster', 'groupcode')],
    ...
    private function outletProductCodeOptions(): array
    {
        $query = DB::table('groupmaster');

        if (Schema::hasColumn('groupmaster', 'activestatus')) {
            $query->where('activestatus', 1);
        }

        return $query
            ->orderBy('groupcode')
            ->get(['groupcode', 'groupname'])
            ->map(fn ($record) => [
                'id' => (int) $record->groupcode,
                'label' => trim($record->groupcode . ' -- ' . ($record->groupname ?? '')),
            ])
            ->all();
    }
    ...
    private function outletScopedQuery(int $fillBy, int $filterValue, int $categoryId)
    {
        return $this->customerScopeQuery($fillBy, $filterValue, $categoryId)
    ...
    private function hasRequiredTables(): bool
    {
        return Schema::hasTable('customermaster')
            ...
            && Schema::hasColumn('customermaster', 'outletsubtype');
    }
}
