...
class SurveyController extends Controller
{
    ...
    {
        return Inertia::render('links/survey/Index', [
            'available' => $this->hasRequiredTables(),
            'formMeta' => [
                'title' => 'Survey',
                'subtitle' => 'Assign a survey key to customers by selected region, depot, area, or route',
                'indexUrl' => '/links/survey',
                'loadUrl' => '/links/survey/load',
                'saveUrl' => '/links/survey/save',
                'permission' => 'survey link',
            ],
            'optionSets' => [
                'surveyKeyOptions' => $this->surveyKeyOptions(),
    ...
    public function save(Request $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($surveyKey, $fillBy, $filterValue, $categoryId, $customerIds) {
            // Match the legacy NMWC workflow: clear the survey key only inside the selected filter scope.
            $this->customerScopeQuery($fillBy, $filterValue, $categoryId)
                ->where('cust.surveykey', $surveyKey)
                ->update(['surveykey' => null]);

            if ($customerIds->isNotEmpty()) {
                DB::table('customermaster')
                    ->whereIn('customercode', $customerIds->all())
                    ->update(['surveykey' => $surveyKey]);
            }
        });

        return redirect('/links/survey')->with('success', 'Update Record');
    }

    private function validatedPayload(Request $request, bool $withCustomers): array
    {
        $rules = [
            'survey_key' => ['required', 'integer', Rule::exists('customersurveykey', 'surveykey')],
    ...
    private function surveyKeyOptions(): array
    {
        $query = DB::table('customersurveykey');

        if (Schema::hasColumn('customersurveykey', 'activestatus')) {
            $query->where('activestatus', 1);
        }

        return $query
            ->orderBy('surveykey')
            ->get(['surveykey', 'surveydescription'])
            ->map(fn ($record) => [
                'id' => (int) $record->surveykey,
                'label' => trim($record->surveykey . ' -- ' . ($record->surveydescription ?? '')),
            ])
            ->all();
    }
    ...
    private function customerQuery(int $surveyKey, int $fillBy, int $filterValue, int $categoryId, bool $selectedOnly)
    {
        ...
        if ($selectedOnly) {
            $query->where('cust.surveykey', $surveyKey);
        }
    ...
    private function customerScopeQuery(int $fillBy, int $filterValue, int $categoryId)
    {
        ...
        switch ($fillBy) {
            ...
            default:
                // Preserve the legacy survey link workflow: route scope is driven by routesequence.
                $query->leftJoin('routesequence as rs', 'rs.customercode', '=', 'cust.customercode');
                if ($filterValue > 0) {
                    $query->where('rs.routecode', $filterValue);
                }
                break;
        }
    ...
    private function hasRequiredTables(): bool
    {
        return Schema::hasTable('customermaster')
            && Schema::hasTable('customersurveykey')
            && Schema::hasTable('categorymaster')
            && Schema::hasTable('routemaster')
            && Schema::hasTable('routesequence')
