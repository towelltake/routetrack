...
class AssetsAvailabilityController extends Controller
{
    ...
    public function __invoke(Request $request): Response
    {
        ...
        return Inertia::render('reports/merchandizing-report/AssetsAvailability', [
    ...
    public function exportExcel(Request $request): HttpResponse
    {
        ...
        return ExcelXmlWorkbook::download(
            'assets-availability-' . now()->format('Ymd_His') . '.xls',
    ...
    public function exportPdf(Request $request): View
    {
        ...
        return view('reports.assets-availability-pdf', [
            'rows' => $rows,
    ...
    {
        $user = $request->user();
        abort_unless($user?->hasFormPermission('assets availability'), 403);
    ...
    private function loadAllRows(array $filters, string $sortBy, string $sortDir): Collection
    {
        ...
        return $this->baseQuery($filters, false)
            ...
            ->sort(function (array $left, array $right) use ($sortBy, $sortDir) {
                ...
                foreach (['route_label', 'visit_date_sort', 'visit_time_sort', 'customer_code', 'pos_code'] as $fallback) {
    ...
    private function baseQuery(array $filters, bool $forCustomerOptions)
    {
        ...
        $remarksSelect = Schema::hasColumn('posequipmentchangedetail', 'remarks')
            ? "COALESCE({$pos}.remarks, '')"
            : "''";
        ...
        $query = DB::table('customeroperationscontrol as coc')
            ...
            ->join('posmaster as pm', 'pm.itemcode', '=', 'pos.itemcode')
            ->leftJoin('posinstructions as poi', 'poi.posinstructioncode', '=', 'pos.posaction')
        ...
        return $query->selectRaw("
            ...
            CAST({$pm}.itemcode AS CHAR) as pos_code,
            ...
            COALESCE({$poi}.posinstructionname, '') as posstatus,
            COALESCE({$poi}.arbposinstructionname, '') as arbposstatus,
    ...
    private function transformRow(array $row): array
    {
        ...
        $posStatus = $isArabic
            ? ($row['arbposstatus'] ?? $row['posstatus'] ?? '')
            : ($row['posstatus'] ?? $row['arbposstatus'] ?? '');
    ...
    private function hasRequiredTables(): bool
    {
        return Schema::hasTable('customeroperationscontrol')
            && Schema::hasTable('posequipmentchangedetail')
