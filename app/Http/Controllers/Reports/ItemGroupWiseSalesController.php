...
class ItemGroupWiseSalesController extends Controller
{
    ...
    private const EXPORT_COLUMNS = [
        ...
        'Route' => 'route_label',
        'Item Category' => 'itemgroup_label',
        'Item Code' => 'itemcode',
        'Description' => 'itemdescription',
        'UPC' => 'upc',
        'Sales Qty' => 'salesqty',
        'Sales @ Inv. Price' => 'sales_inv_price',
        'G. Return Qty' => 'returnqty',
        'G. Return @ Inv. Price' => 'good_return_inv_price',
        'Damaged Return Qty' => 'damagedqty',
        'Damaged Return @ Inv. Price' => 'damage_inv_price',
        'Expired Qty' => 'expiryqty',
        'Expired @ Inv. Price' => 'expired_inv_price',
        'Free Qty' => 'freeqty',
    ...
    public function __invoke(Request $request): Response
    {
        ...
        return Inertia::render('reports/data-analysis/ItemGroupWiseSales', [
    ...
    public function exportExcel(Request $request): HttpResponse
    {
        ...
        return ExcelXmlWorkbook::download(
            'item-group-wise-sales-' . now()->format('Ymd_His') . '.xls',
    ...
    public function exportPdf(Request $request): View
    {
        ...
        return view('reports.item-group-wise-sales-pdf', [
            'rows' => $rows,
    ...
    {
        $user = $request->user();
        abort_unless($user?->hasFormPermission('item group wise sales'), 403);
    ...
    {
        if (!$this->hasTables(['invoiceheader', 'invoicedetail', 'itemmaster', 'itemgroup', 'routemaster'])) {
            return collect();
        }
        ...
        return collect(array_values($grouped))
            ->sort(function (array $left, array $right) use ($sortBy, $sortDir) {
                ...
                foreach (['transactiondate_sort', 'routecode_sort', 'itemgroupcode_sort', 'itemcode_sort'] as $fallback) {
    ...
    private function baseQuery(array $filters)
    {
        ...
        return $query
            ...
            ->leftJoin('routemaster as rm', 'rm.routecode', '=', 'ih.routecode')
            ->when(
            ...
            ->selectRaw("
                " . ($joinWithTransactionKey ? "{$ih}.transactionkey as transactionkey" : 'CONCAT(CAST(' . $ih . '.routekey AS CHAR), \'-\', CAST(' . $ih . '.visitkey AS CHAR)) as transactionkey') . ",
                DATE_FORMAT({$dateColumn}, '%d %b %Y') as transactiondate,
                DATE_FORMAT({$dateColumn}, '%Y-%m-%d') as transactiondate_sort,
                COALESCE({$ih}.routecode, 0) as routecode_sort,
    ...
    private function transformLine(array $row, float $invoiceSalesTotal): array
    {
        ...
        $discounts = $invoiceDiscountBreakup + $itemDiscountBreakup;
        ...
        return [
            ...
            'route_label' => trim($this->identifier($row['routecode_display'] ?? '') . ($routeName !== '' ? ' - ' . $routeName : '')),
            ...
            'free_goods_std_price' => (float) ($row['free_goods_std_price'] ?? 0),
            'discounts' => $discounts,
            'total_amount' => $salesInvPrice + $goodReturnInvPrice + $damageInvPrice + $expiredInvPrice + $discounts,
        ];
    }
    ...
    private function totals(Collection $rows): array
    {
        ...
        foreach (array_values(self::EXPORT_COLUMNS) as $key) {
            if (in_array($key, ['transactiondate', 'route_label', 'itemgroup_label', 'itemcode', 'itemdescription'], true)) {
                continue;
            }
    ...
    private function selectedOptionLabel(array $options, mixed $value): string
    {
        if ($value === null || $value === '') {
            return 'All';
        }

        $match = collect($options)->first(fn (array $option) => (int) ($option['id'] ?? 0) === (int) $value);
