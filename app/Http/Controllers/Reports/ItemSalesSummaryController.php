...
class ItemSalesSummaryController extends Controller
{
    private const SORT_COLUMNS = [
        ...
        'expired_std_price',
        'expired_inv_price',
        'good_return_std_price',
        'good_return_inv_price',
    ...
    private const EXPORT_COLUMNS = [
        ...
        'Item Group Code' => 'itemgroup_label',
        'Item Code' => 'itemcode',
        'Item Description' => 'itemdescription',
        'UPC' => 'upc',
        'Sales Qty' => 'salesqty',
        'Free Qty' => 'freeqty',
        'Damage Ret. Qty' => 'damagedqty',
        'Expired Qty' => 'expiryqty',
        'Good Ret. Qty' => 'returnqty',
        'Sales @ Std. Price' => 'sales_std_price',
        'Sales @ Inv. Price' => 'sales_inv_price',
        'Damage Ret @ Std. Price' => 'damage_std_price',
        'Damage Ret @ Inv. Price' => 'damage_inv_price',
        'Expired Ret @ Std. Price' => 'expired_std_price',
        'Expired Ret @ Inv. Price' => 'expired_inv_price',
        'Good Return @ Std. Price' => 'good_return_std_price',
        'Good Return @ Inv. Price' => 'good_return_inv_price',
    ...
    public function __invoke(Request $request): Response
    {
        ...
        return Inertia::render('reports/data-analysis/ItemSalesSummary', [
    ...
    public function exportExcel(Request $request): HttpResponse
    {
        ...
        return ExcelXmlWorkbook::download(
            'item-sales-summary-' . now()->format('Ymd_His') . '.xls',
    ...
    public function exportPdf(Request $request): View
    {
        ...
        return view('reports.item-sales-summary-pdf', [
            'rows' => $rows,
    ...
    {
        $user = $request->user();
        abort_unless($user?->hasFormPermission('item sales summary'), 403);
        ...
        $requestedSortBy = $validated['sort_by'] ?? 'majorcategorycode_sort';

        return [
            'filters' => $filters,
            'scope' => $scope,
            'sort_by' => in_array($requestedSortBy, self::SORT_COLUMNS, true) ? $requestedSortBy : 'majorcategorycode_sort',
    ...
    {
        if (!$this->hasTables(['invoiceheader', 'invoicedetail', 'itemmaster', 'itemgroup'])) {
            return collect();
        }
        ...
        return collect(array_values($grouped))
            ->sort(function (array $left, array $right) use ($sortBy, $sortDir) {
                ...
                foreach (['transactiondate_sort', 'majorcategorycode_sort', 'itemgroupcode_sort', 'itemcode_sort'] as $fallback) {
    ...
    private function baseQuery(array $filters)
    {
        ...
        $expiryStdValueExpression = $this->valueExpression("COALESCE({$id}.expiryqty, 0)", $id, $im, 'stdsalescaseprice', 'stdsalesprice');
        $expiryInvValueExpression = $this->valueExpression("COALESCE({$id}.expiryqty, 0)", $id, $im, 'salescaseprice', 'salesprice');
        ...
        return $query
            ...
            ->selectRaw("
                ...
                COALESCE({$ig}.itemgroupcode, 0) as itemgroupcode_sort,
                COALESCE({$ig}.itemgroupname, '') as itemgroupname,
                COALESCE({$ig}.arbitemgroup, '') as arbitemgroup,
                ...
                COALESCE({$id}.expiryqty, 0) as expiryqty,
                ...
                {$expiryStdValueExpression} as expiry_std_base,
                {$expiryInvValueExpression} as expiry_inv_base,
    ...
    private function transformLine(array $row, float $invoiceSalesTotal): array
    {
        ...
        $itemGroup = $isArabic
            ? ($row['arbitemgroup'] ?? $row['itemgroupname'] ?? '')
            : ($row['itemgroupname'] ?? $row['arbitemgroup'] ?? '');
        ...
        $damageStdPrice = -1 * (float) ($row['damage_std_base'] ?? 0);
        $damageInvPrice = -1 * (float) ($row['damage_inv_base'] ?? 0);
        $expiredStdPrice = -1 * (float) ($row['expiry_std_base'] ?? 0);
        $expiredInvPrice = -1 * (float) ($row['expiry_inv_base'] ?? 0);
        $goodReturnStdPrice = -1 * (float) ($row['good_return_std_base'] ?? 0);
        $goodReturnInvPrice = -1 * (float) ($row['good_return_inv_base'] ?? 0);
        ...
        return [
            ...
            'itemgroupcode_sort' => (int) ($row['itemgroupcode_sort'] ?? 0),
            'itemgroup_label' => trim((((int) ($row['itemgroupcode_sort'] ?? 0)) > 0 ? ((int) ($row['itemgroupcode_sort'] ?? 0)) . ' - ' : '') . $itemGroup),
            ...
            'expiryqty' => (float) ($row['expiryqty'] ?? 0),
            'returnqty' => (float) ($row['returnqty'] ?? 0),
            'sales_std_price' => $salesStdPrice,
            'sales_inv_price' => $salesInvPrice,
            'damage_std_price' => $damageStdPrice,
            'damage_inv_price' => $damageInvPrice,
            'expired_std_price' => $expiredStdPrice,
            'expired_inv_price' => $expiredInvPrice,
            'good_return_std_price' => $goodReturnStdPrice,
            'good_return_inv_price' => $goodReturnInvPrice,
            ...
            'net_sales' => $salesInvPrice + $damageInvPrice + $expiredInvPrice + $goodReturnInvPrice + $invoiceDiscountBreakup + $itemDiscountBreakup,
    ...
    private function totals(Collection $rows): array
    {
        ...
        foreach (array_values(self::EXPORT_COLUMNS) as $key) {
            if (in_array($key, ['transactiondate', 'majorcategory_label', 'itemgroup_label', 'itemcode', 'itemdescription'], true)) {
                continue;
            }
