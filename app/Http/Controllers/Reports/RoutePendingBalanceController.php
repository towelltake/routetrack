...
class RoutePendingBalanceController extends Controller
{
    ...
    public function __invoke(Request $request): Response
    {
        ...
        return Inertia::render('reports/accounts-report/RoutePendingBalance', [
    ...
    public function exportExcel(Request $request): HttpResponse
    {
        ...
        return ExcelXmlWorkbook::download(
            'route-pending-balance-' . now()->format('Ymd_His') . '.xls',
            $headers,
            $exportRows,
    ...
    public function exportPdf(Request $request): View
    {
        ...
        return view('reports.route-pending-balance-pdf', [
            'rows' => $rows,
            'periodColumns' => $context['period_columns'],
    ...
    {
        $user = $request->user();
        abort_unless($user?->hasFormPermission('route pending balance'), 403);
    ...
    private function loadAllRows(array $filters, string $sortBy, string $sortDir): Collection
    {
        ...
        foreach ($this->baseQuery($filters)->get() as $row) {
            ...
            if (array_key_exists($year, $pivoted[$rowKey]['period_values'])) {
                $pivoted[$rowKey]['period_values'][$year] += $amount;
            }
        ...
        return collect(array_values($pivoted))
            ...
            ->sort(function (array $left, array $right) use ($sortBy, $sortDir, $periodColumns) {
                $leftValue = $left[$sortBy] ?? null;
                $rightValue = $right[$sortBy] ?? null;
                $baseCompare = $this->compare($leftValue, $rightValue);
                if ($baseCompare !== 0) {
    ...
    private function baseQuery(array $filters)
    {
        ...
        return DB::table('customerinvoice as ci')
            ...
            ->when(
                ...
                fn ($builder) => $builder->where('ci.invoicebalance', '<>', 0)
            )
            ->when(
                $filters['routecodes'] !== [],
            ...
            ->selectRaw("
                ...
                SUM(COALESCE({$ci}.invoicebalance, 0)) as invoicebalance
            ")
            ->groupByRaw("
    ...
    private function periodColumns(array $filters): array
    {
        ...
        return collect($this->yearOptions($filters))->pluck('id')->values()->all();
    }
