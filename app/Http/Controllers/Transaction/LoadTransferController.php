...
class LoadTransferController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        if (!Schema::hasTable('inventorytransactionheader') || !Schema::hasTable('inventorytransactiondetail')) {
            return Inertia::render('transaction/load-transfer/Index', [
        ...
        $query = DB::table("inventorytransactionheader as {$headerAlias}")
            ...
            ->whereExists(function ($subQuery) {
                ...
                if (Schema::hasColumn('inventorytransactiondetail', 'transactiontypecode')) {
                    $subQuery->whereIn('detail.transactiontypecode', array_keys(self::LINE_TYPES));
                }
            });
        ...
        if (Schema::hasColumn('inventorytransactionheader', 'transactiontype')) {
            $query->where("{$headerAlias}.transactiontype", 2);
        }
    ...
    public function show(Request $request, int $detailkey): Response
    {
        ...
        $routeAlias = 'route';
        $salesmanAlias = 'salesman';
        $transferRouteAlias = 'transfer_route';
        $detailAlias = 'detail';
        $itemAlias = 'item';
        $qualifiedHeaderAlias = DB::getTablePrefix() . $headerAlias;
        $qualifiedRouteAlias = DB::getTablePrefix() . $routeAlias;
        $qualifiedSalesmanAlias = DB::getTablePrefix() . $salesmanAlias;
        $qualifiedTransferRouteAlias = DB::getTablePrefix() . $transferRouteAlias;
        $qualifiedDetailAlias = DB::getTablePrefix() . $detailAlias;
        $qualifiedItemAlias = DB::getTablePrefix() . $itemAlias;

        $selectedLineType = $this->selectedLineType($request);

        $header = DB::table("inventorytransactionheader as {$headerAlias}")
            ->leftJoin("routemaster as {$routeAlias}", "{$routeAlias}.routecode", '=', "{$headerAlias}.routecode")
            ->leftJoin("salesman as {$salesmanAlias}", "{$salesmanAlias}.salesmancode", '=', "{$headerAlias}.salesmancode")
            ->leftJoin("routemaster as {$transferRouteAlias}", "{$transferRouteAlias}.routecode", '=', "{$headerAlias}.transferlocationcode")
            ...
            ->selectRaw('
                ...
                ' . $this->columnExpression('inventorytransactionheader', 'loadnumber', $qualifiedHeaderAlias . '.loadnumber') . ' as loadnumber,
                ' . $this->columnExpression('inventorytransactionheader', 'transferlocationcode', $qualifiedHeaderAlias . '.transferlocationcode') . ' as transferlocationcode,
                COALESCE(' . $qualifiedTransferRouteAlias . '.routename, "") as transferroutename,
                COALESCE(' . $qualifiedTransferRouteAlias . '.arbroutename, "") as arbtransferroutename
            ')
            ->first();
        ...
        $lineQuery = DB::table("inventorytransactiondetail as {$detailAlias}")
            ...
            ->selectRaw('
                ...
                COALESCE(' . $qualifiedDetailAlias . '.transactiontypecode, 0) as transactiontypecode,
        ...
        if (Schema::hasColumn('inventorytransactiondetail', 'transactiontypecode')) {
            $lineQuery->where("{$detailAlias}.transactiontypecode", $selectedLineType);
        }
        ...
        return Inertia::render('transaction/load-transfer/Show', [
            'header' => [
                ...
                'transferlocationcode' => is_numeric($header->transferlocationcode ?? null)
                    ? (int) $header->transferlocationcode
                    : 0,
                'transferroutename' => $header->transferroutename ?? '',
                'arbtransferroutename' => $header->arbtransferroutename ?? '',
            ],
            'lines' => $lines,
            'lineTypeOptions' => collect(self::LINE_TYPES)->map(fn ($label, $value) => [
                'value' => (int) $value,
                'label' => $label,
            ])->values()->all(),
    ...
    private function selectedLineType(Request $request): int
    {
        $lineType = (int) $request->input('line_type', 3);

        return array_key_exists($lineType, self::LINE_TYPES) ? $lineType : 3;
    }
