...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $existing = DB::table('moduledetail')
            ->whereRaw('LOWER(TRIM(formname)) = ?', ['special price link'])
            ->first();

        $description = 'Assign pricing keys to customers';

        if ($existing) {
            DB::table('moduledetail')->where('formid', $existing->formid)->update([
                'moduleid' => $moduleId,
                'formname' => 'Special Price Link',
        ...
        $source = DB::table('moduledetail')
            ->whereRaw('LOWER(TRIM(formname)) = ?', ['pricing key'])
            ->first()
        ...
        if ($source) {
            ...
            $payload['formname'] = 'Special Price Link';
