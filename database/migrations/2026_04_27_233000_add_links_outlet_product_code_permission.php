...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $existing = DB::table('moduledetail')
            ->whereRaw('LOWER(TRIM(formname)) = ?', ['outlet product code'])
            ->first();

        $description = 'Assign outlet product codes to customers';

        if ($existing) {
            DB::table('moduledetail')->where('formid', $existing->formid)->update([
                'moduleid' => $moduleId,
                'formname' => 'Outlet Product Code',
        ...
        $source = DB::table('moduledetail')
            ->whereRaw('LOWER(TRIM(formname)) = ?', ['promotion link'])
            ->first()
        ...
        if ($source) {
            ...
            $payload['formname'] = 'Outlet Product Code';
