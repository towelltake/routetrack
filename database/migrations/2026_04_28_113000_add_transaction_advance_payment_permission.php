...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $description = 'Manage advance payment transactions';
        ...
        if ($existing) {
            DB::table('moduledetail')
                ...
                ->update([
                    'moduleid' => $moduleId,
                    'formname' => 'Advance Payment',
                    'formdescription' => $description,
                ]);

            $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Advance Payment', $description);
            $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Advance Payment', $description);
            return;
        }
        ...
        $payload['formname'] = 'Advance Payment';
        $payload['formdescription'] = $description;
        $payload['order'] = $nextOrder;

        DB::table('moduledetail')->insert($payload);

        $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, $nextFormId, $moduleId, 'Advance Payment', $description);
        $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, $nextFormId, $moduleId, 'Advance Payment', $description);
    }
