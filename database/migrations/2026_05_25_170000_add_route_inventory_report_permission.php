...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (!$source) {
            $source = DB::table('moduledetail')
                ->whereRaw('LOWER(TRIM(formname)) = ?', ['sales summary'])
                ->first();
        }
        ...
        if ($existing) {
            DB::table('moduledetail')
                ...
                ->update([
                    'moduleid' => $moduleId,
                    'formname' => 'Route Inventory',
                    'formdescription' => $description,
                ]);

            if ($source) {
                $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Route Inventory', $description);
                $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Route Inventory', $description);
            }
        ...
        if ($source) {
            ...
            $payload['formname'] = 'Route Inventory';
            $payload['formdescription'] = $description;
            $payload['order'] = $nextOrder;

            DB::table('moduledetail')->insert($payload);

            $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, $nextFormId, $moduleId, 'Route Inventory', $description);
            $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, $nextFormId, $moduleId, 'Route Inventory', $description);
            return;
        }
