...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $source = DB::table('moduledetail')
            ->whereRaw('LOWER(TRIM(formname)) = ?', ['route activity'])
            ->first();

        if (!$source) {
            $source = DB::table('moduledetail')
                ->whereRaw('LOWER(TRIM(formname)) = ?', ['route inventory'])
                ->first();
        }

        if (!$source) {
            $source = DB::table('moduledetail')
                ->whereRaw('LOWER(TRIM(formname)) = ?', ['route summary'])
                ->first();
        }

        $description = 'Route Trip Analysis';
        ...
        if ($existing) {
            DB::table('moduledetail')
                ...
                ->update([
                    'moduleid' => $moduleId,
                    'formname' => 'Route Trip Analysis',
                    'formdescription' => $description,
                ]);

            if ($source) {
                $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Route Trip Analysis', $description);
                $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Route Trip Analysis', $description);
            }
        ...
        if ($source) {
            ...
            $payload['formname'] = 'Route Trip Analysis';
            $payload['formdescription'] = $description;
            $payload['order'] = $nextOrder;

            DB::table('moduledetail')->insert($payload);

            $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, $nextFormId, $moduleId, 'Route Trip Analysis', $description);
            $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, $nextFormId, $moduleId, 'Route Trip Analysis', $description);
            return;
        }
