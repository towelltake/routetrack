...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $description = 'Route Deposit Summary';
        ...
        if ($existing) {
            DB::table('moduledetail')
                ...
                ->update([
                    'moduleid' => $moduleId,
                    'formname' => 'Route Deposit Summary',
                    'formdescription' => $description,
                ]);

            if ($source) {
                $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Route Deposit Summary', $description);
                $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Route Deposit Summary', $description);
            }
        ...
        if ($source) {
            ...
            $payload['formname'] = 'Route Deposit Summary';
            $payload['formdescription'] = $description;
            $payload['order'] = $nextOrder;

            DB::table('moduledetail')->insert($payload);

            $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, $nextFormId, $moduleId, 'Route Deposit Summary', $description);
            $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, $nextFormId, $moduleId, 'Route Deposit Summary', $description);
            return;
        }
