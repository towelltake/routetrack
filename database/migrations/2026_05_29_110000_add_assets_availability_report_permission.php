...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $existing = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['assets availability'])->first();
        $source = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['waste stock'])->first()
            ?: DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['survey tracking'])->first()
            ?: DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['pos tracking'])->first();
        $description = 'Assets Availability';

        if ($existing) {
            DB::table('moduledetail')->where('formid', $existing->formid)->update(['moduleid' => $moduleId, 'formname' => 'Assets Availability', 'formdescription' => $description]);
            if ($source) {
                $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Assets Availability', $description);
                $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Assets Availability', $description);
            }
            return;
        }
        ...
        if ($source) {
            ...
            $payload['formname'] = 'Assets Availability';
            $payload['formdescription'] = $description;
            $payload['order'] = $nextOrder;
            DB::table('moduledetail')->insert($payload);
            $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, $nextFormId, $moduleId, 'Assets Availability', $description);
            $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, $nextFormId, $moduleId, 'Assets Availability', $description);
            return;
        }

        DB::table('moduledetail')->insert(['formid' => $nextFormId, 'moduleid' => $moduleId, 'formname' => 'Assets Availability', 'formdescription' => $description, 'order' => $nextOrder]);
    }
    ...
    public function down(): void
    {
        ...
        $formId = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['assets availability'])->value('formid');
        if (!$formId) {
            return;
        }
