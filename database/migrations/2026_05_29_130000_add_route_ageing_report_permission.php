...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $existing = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['route ageing'])->first();
        $source = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['payment summary'])->first()
        ...
        $description = 'Route Ageing';

        if ($existing) {
            DB::table('moduledetail')->where('formid', $existing->formid)->update(['moduleid' => $moduleId, 'formname' => 'Route Ageing', 'formdescription' => $description]);
            if ($source) {
                $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Route Ageing', $description);
                $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Route Ageing', $description);
            }
            return;
        }
        ...
        if ($source) {
            ...
            $payload['formname'] = 'Route Ageing';
            $payload['formdescription'] = $description;
            $payload['order'] = $nextOrder;
            DB::table('moduledetail')->insert($payload);
            $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, $nextFormId, $moduleId, 'Route Ageing', $description);
            $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, $nextFormId, $moduleId, 'Route Ageing', $description);
            return;
        }

        DB::table('moduledetail')->insert(['formid' => $nextFormId, 'moduleid' => $moduleId, 'formname' => 'Route Ageing', 'formdescription' => $description, 'order' => $nextOrder]);
    }
    ...
    public function down(): void
    {
        ...
        $formId = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['route ageing'])->value('formid');
        if (!$formId) {
            return;
        }
