...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $existing = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['sales free summary'])->first();
        $source = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['route monthly revenue'])->first()
            ?: DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['customer pending balance'])->first()
            ?: DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['route pending balance'])->first();
        $description = 'Sales Free Summary';

        if ($existing) {
            DB::table('moduledetail')->where('formid', $existing->formid)->update([
                'moduleid' => $moduleId,
                'formname' => 'Sales Free Summary',
                'formdescription' => $description,
            ]);
            if ($source) {
                $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Sales Free Summary', $description);
                $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Sales Free Summary', $description);
            }
            return;
        }
        ...
        if ($source) {
            ...
            $payload['formname'] = 'Sales Free Summary';
            $payload['formdescription'] = $description;
            $payload['order'] = $nextOrder;
            DB::table('moduledetail')->insert($payload);
            $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, $nextFormId, $moduleId, 'Sales Free Summary', $description);
            $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, $nextFormId, $moduleId, 'Sales Free Summary', $description);
            return;
        }
    ...
    public function down(): void
    {
        ...
        $formId = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['sales free summary'])->value('formid');
        if (!$formId) {
            return;
        }
