...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $existing = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['item sales summary'])->first();
        $source = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['sales free summary'])->first()
            ?: DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['route monthly revenue'])->first()
        ...
        if ($existing) {
            DB::table('moduledetail')->where('formid', $existing->formid)->update([
                'moduleid' => $moduleId,
                'formname' => 'Item Sales Summary',
                'formdescription' => $description,
            ]);
            if ($source) {
                $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Item Sales Summary', $description);
                $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Item Sales Summary', $description);
            }
            return;
        }
        ...
        if ($source) {
            ...
            $payload['formname'] = 'Item Sales Summary';
            $payload['formdescription'] = $description;
            $payload['order'] = $nextOrder;
            DB::table('moduledetail')->insert($payload);
            $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, $nextFormId, $moduleId, 'Item Sales Summary', $description);
            $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, $nextFormId, $moduleId, 'Item Sales Summary', $description);
            return;
        }
    ...
    public function down(): void
    {
        ...
        $formId = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['item sales summary'])->value('formid');
        if (!$formId) {
            return;
        }
