...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $existing = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['item group wise sales'])->first();
        $source = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['item sales summary'])->first()
            ?: DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['sales free summary'])->first();
        $description = 'Item Group Wise Sales';

        if ($existing) {
            DB::table('moduledetail')->where('formid', $existing->formid)->update([
                'moduleid' => $moduleId,
                'formname' => 'Item Group Wise Sales',
                'formdescription' => $description,
            ]);
            if ($source) {
                $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Item Group Wise Sales', $description);
                $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Item Group Wise Sales', $description);
            }
            return;
        }
        ...
        if ($source) {
            ...
            $payload['formname'] = 'Item Group Wise Sales';
            $payload['formdescription'] = $description;
            $payload['order'] = $nextOrder;
            DB::table('moduledetail')->insert($payload);
            $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, $nextFormId, $moduleId, 'Item Group Wise Sales', $description);
            $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, $nextFormId, $moduleId, 'Item Group Wise Sales', $description);
            return;
        }
    ...
    public function down(): void
    {
        ...
        $formId = DB::table('moduledetail')->whereRaw('LOWER(TRIM(formname)) = ?', ['item group wise sales'])->value('formid');
        if (!$formId) {
            return;
        }
