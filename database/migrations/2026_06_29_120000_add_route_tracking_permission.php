...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $moduleId = (int) DB::table('moduleheader')
            ->whereRaw('LOWER(TRIM(modulename)) = ?', ['route tracking'])
            ->value('moduleid');
        ...
        if ($moduleId <= 0) {
            ...
            DB::table('moduleheader')->insert([
                'moduleid' => $moduleId,
                'modulename' => 'Route Tracking',
            ]);
        }

        $existing = DB::table('moduledetail')
            ->whereRaw('LOWER(TRIM(formname)) = ?', ['route tracking'])
            ->first();
        ...
        if ($existing) {
            DB::table('moduledetail')
                ...
                ->update([
                    'moduleid' => $moduleId,
                    'formname' => 'Route Tracking',
                    'formdescription' => $description,
                ]);

            if ($source) {
                $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Route Tracking', $description);
                $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, (int) $existing->formid, $moduleId, 'Route Tracking', $description);
            }
        ...
        $payload['formname'] = 'Route Tracking';
        $payload['formdescription'] = $description;
        $payload['order'] = $nextOrder;

        DB::table('moduledetail')->insert($payload);

        $this->syncPermissionRows('userdetail', 'userid', (int) $source->formid, $nextFormId, $moduleId, 'Route Tracking', $description);
        $this->syncPermissionRows('usertypedetail', 'usertypeid', (int) $source->formid, $nextFormId, $moduleId, 'Route Tracking', $description);
    }
