...
if (!defined('PHPEXCEL_ROOT')) {
	define('PHPEXCEL_ROOT',APP_LIBRARY_PATH . '/');
...
// HC2 - 19 June
// This function is used to open & save .xls file in IE & other browsers
function includeHeaderContent($file_title, $objPHPExcel)
{
...
/** Change Pos for the excel sheet cell */
function changepos($alpha,$operation="",$num=0)
{    
    if($operation!="")
    {
        if($operation == "+")
        {
            //$alpha = chr(ord($alpha)+$num);
            $val = ord($alpha)+$num;
            if($val > ord("Z"))
            {
                $newnum = ord("Z") - ord($alpha);
                $diff = $num - $newnum;
                $newaddedchar = ord("A") + ($diff-1);
                $newalpha = "A".chr($newaddedchar);
            }
            else
            {
                $newalpha = chr(ord($alpha)+$num);
            }
        }
        else if($operation == "-")
        {
...
/** Remove the HTML Breaks from the data */
function breakdata_from_html($datashow)
{
	$returndata = array();
	
	if(strpos($datashow,"<br >")!==false)
	{
		$datashow = str_replace("<br >","<br>",$datashow);
	}
	...
	return $returndata = explode("<br>",$datashow);
	
}


/** Remove the HTML Breaks from the data */
function breakdata_from_html_to_enter($datashow)
{
	$returndata = array();
	...
	return $returndata = str_replace("<br>",chr(10),$datashow);
