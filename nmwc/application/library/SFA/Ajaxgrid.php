    ...
    public function create_data_grid($data_row_display)
    {
        ...
        for($s=0; $s<count($data_row_display); $s++)
        {
            ...
            {   
                ...
                if($this->configuration_params['show_extratextbox'])
                {
                    $final_data_arr[$s] = array_slice($data_row_display[$s], 0, 0, true) +  array("selectbox" => '<input type="checkbox" class="checkbox sel_checkbox_all" name="chk[]"  id="chk_'.$data_row_display[$s]['invoicenumber'].'" onclick="toggleUnChecked(this.checked,this.value);" value="'.$edit_delete_primary_id.'" />') +   array_slice($data_row_display[$s], 0, count($data_row_display[$s]) - 1, true);
                }
                else
                {
                    $final_data_arr[$s] = array_slice($data_row_display[$s], 0, 0, true) +  array("selectbox" => '<input type="checkbox" class="checkbox sel_checkbox_all" name="chk[]"  onclick="toggleUnChecked(this.checked,this.value);" value="'.$edit_delete_primary_id.'" />') +   array_slice($data_row_display[$s], 0, count($data_row_display[$s]) - 1, true);
                }
            }
