	...
	for($s=0; $s<count($data_row_display); $s++)
	{
	    ...
	    if($configuration_params['show_selectbox'])
	    {
		   if(isset($configuration_params["selected_list"]) && count($configuration_params["selected_list"]) && in_array($edit_delete_primary_id,$configuration_params["selected_list"]))
			...
			{
				$final_data_arr[$s] = array_slice($data_row_display[$s], 0, 0, true) +  array("selectbox" => '<input type="checkbox" class="checkboxcustomer sel_checkbox_all" name="chk_customer[]"  id="chk_'.$data_row_display[$s]['invoicenumber'].'" onclick="toggleUnCheckedCustomer(this.checked,this.value);" value="'.$edit_delete_primary_id.'" ' .$tmp_checked. ' />') +   array_slice($data_row_display[$s], 0, count($data_row_display[$s]), true);
			}
			else
			{
				$final_data_arr[$s] = array_slice($data_row_display[$s], 0, 0, true) +  array("selectbox" => '<input type="checkbox" class="checkboxcustomer sel_checkbox_all" name="chk_customer[]"  onclick="toggleUnCheckedCustomer(this.checked,this.value);" value="'.$edit_delete_primary_id.'" ' .$tmp_checked. ' />') +   array_slice($data_row_display[$s], 0, count($data_row_display[$s]), true);
			}
	    }
