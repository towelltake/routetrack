...
class SFA_Ajaxpagingitem{
	...
	for($s=0; $s<count($data_row_display); $s++)
	{
	    ...
	    if($configuration_params['show_selectbox'])
	    {
		   if(isset($configuration_params["selected_list"]) && count($configuration_params["selected_list"]) && in_array($edit_delete_primary_id,$configuration_params["selected_list"]))
		    ...
		    // Added By Hiren dave for adding id in checkbox
			if($configuration_params['show_extratextbox'])
			{
				$final_data_arr[$s] = array_slice($data_row_display[$s], 0, 0, true) +  array("selectbox" => '<input type="checkbox" class="checkboxitem sel_checkbox_all" name="chk_item[]"  id="chk_'.$data_row_display[$s]['invoicenumber'].'" onclick="toggleUnCheckedItem(this.checked,this.value);" value="'.$edit_delete_primary_id.'" ' .$tmp_checked. ' />') +   array_slice($data_row_display[$s], 0, count($data_row_display[$s]), true);
			}
			else
			{
				$final_data_arr[$s] = array_slice($data_row_display[$s], 0, 0, true) +  array("selectbox" => '<input type="checkbox" class="checkboxitem sel_checkbox_all" name="chk_item[]"  onclick="toggleUnCheckedItem(this.checked,this.value);" value="'.$edit_delete_primary_id.'" ' .$tmp_checked. ' />') +   array_slice($data_row_display[$s], 0, count($data_row_display[$s]), true);
			}
	    }
		 ...
		 */
		//if($data_row_display[$s]['used_foc'] > 0)
			//$editlink = '<a onclick="jAlert(\'Contract Already Started For This Item; Cant Edit Now.\');" class="">Edit</a>';
		//else
			$editlink = '<a href="javascript:void(0)" id="'.$editlink.'" class="ico edit xedit" >'.$this->translate->_('Edit').'</a>';
		/*
		...
		$deletelink = '<a href="Javascript:void(0)" id="'.$deletelink.'" class="icodel" >'.$this->translate->_('Delete').'</a>';
		...
		// Added By Hiren Dave
		// The below link is use for the open popup
		if($configuration_params['show_extralink_popup'])
	    {
			$ex_counter = 0;
			if($data_row_display[$s]['sett_tran_type'] == 1)
			{
				$extral_link_array = $configuration_params['extralink'];				
				foreach($extral_link_array as $_el)
				{
					 ...
					 */
					//$deletelink = '<a href="'.$baseurl.$deletelink.'" class="ask" onclick="return confirm(\'Are you sure you want to delete?\')"><img src="'.$baseurl.'/public/pagegrid_images/trash.png" alt="Delete" title="Delete" border="0" /></a>';
					$f_extralink = '<a rel="prettyPhoto[iframe]" id="extra_'.$ex_counter."_".$edit_delete_primary_id.'" href="'.$baseurl.$extralink.'" >'.$_el[0].'</a>';
					/*
					 * Fill anchor tag in predefined data array
					 */
					$final_data_arr[$s]['extra_link'.$ex_counter] = $f_extralink;					
			...
			{
				$final_data_arr[$s]['extra_link'.$ex_counter] = '&nbsp';
			}
	    }
		
		if($configuration_params['show_extratextbox'])
	    {	
			$f_extratextbox = '<input type="text" readonly="readonly" onblur="checkinvobal(this.value,this.id)" class="number textbox_in_grid" id="extratextbox_'.$data_row_display[$s]['invoicenumber'].'" name="extratextbox_'.$data_row_display[$s]['invoicenumber'].'" value="" class="icoextratextbox" />';
            $final_data_arr[$s]['extra_textbox'.$s] = $f_extratextbox;
	    }
    ...
    */
	...
	(
	     ...
	     // Top and top value this two parmeter is use for display merge colum to top
	     //like  below example
	     /*
		    Sales            Product
		   -----------------------------------------------------
		   S.Price | S.No    P.No | P.Name
		
		another parmeter columns_total,   columns_total_value,columns_total_msg is
		use for display last summation row if colum totle value is yes then calculate the sum
	     */
	      ...
	      "columns_top_value" =>$this->configuration_params["show_top_columns_value"],
	       ...
	       "columns_total" =>$this->configuration_params["show_total_columns"],
	     
	      "columns_total_value" =>$this->configuration_params["show_total_columns_value"],
	      
	      "columns_total_msg" =>$this->configuration_params["show_total_columns_msg"],
	      
	        "columns_show_right" =>$this->configuration_params["show_columns_right_side"],
