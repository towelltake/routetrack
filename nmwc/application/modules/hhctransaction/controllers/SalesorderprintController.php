...
class Hhctransaction_SalesorderprintController extends Hhctransaction_Library_Controller_Action_Abstract
{
    ...
    */
	  ...
	  /**/
		...
		/*$columns_array		= array('im.actualitemcode','im.itemshortdescription','im.unitspercase',
			...
			$columns_array 	=  array('im.actualitemcode','im.itemshortdescription','im.unitspercase',
						...
						'FORMAT(id.salesorderexcisetax,'.$this->decimalplaces.') AS sales_excise',
						'FORMAT(id.salesordervat,'.$this->decimalplaces.') AS sales_vat',	

						'FORMAT(id.salesorderexcisetax,'.$this->decimalplaces.') AS return_excise',
						'FORMAT(id.returnvat,'.$this->decimalplaces.') AS return_vat',	
						
						'FORMAT(id.damagedexcisetax,'.$this->decimalplaces.') AS bad_return_excise',
						'FORMAT(id.damagedvat,'.$this->decimalplaces.') AS bad_return_vat',			
						
						'FORMAT(id.promoexcisetax,'.$this->decimalplaces.') AS promo_excise',
						'FORMAT(id.promovat,'.$this->decimalplaces.') AS promo_vat',

						'FORMAT(id.fgexcisetax,'.$this->decimalplaces.') AS free_excise',
						'FORMAT(id.fgvat,'.$this->decimalplaces.') AS free_vat',			
	...
	for($j=0;$j<count($column_model_arr);$j++){
			...
			if($column_model_arr[0]['itemtaxkey1'] > 0){	 
			$html .=' <td style="text-align:right">';			
			$html .=  $column_model_arr[$j]['free_excise']+$column_model_arr[$j]['promo_excise'];			
			$html .='  </td>';
			}
			  
			$html .=' <td style="text-align:right">';			
			$html .=  $column_model_arr[$j]['free_vat']+$column_model_arr[$j]['promo_vat'];			
			$html .='  </td>';
	...
	if($invoice_header['totallineitemtax'] != 0){
	$html .='<tr>'; 
	$html .='<td >';
	$html .= "<b> </b>";
	$html .='</td>';
	...
	$html .='</td>';
	$html .='<td >';
	$html .= number_format($invoice_header['totallineitemtax'],$this->decimalplaces);
	$html .='</td>';
	$html .='</tr>'; 
	$html .='<tr>'; 
	}
