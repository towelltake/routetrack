		...
		*/
		...
		if($formdata['ddlitemgrp'] != "" && $formdata['txtitemname'] != "" && $formdata['txtdescription'] != "")
		{
			...
			$param_array = array(
			   ...
			   21   => ($formdata["chkshelfstock"] <>""?trim($formdata["chkshelfstock"]):0), 	//captureshelfstock
			   22   => ($formdata["chktcallowed"] <> ""?trim($formdata["chktcallowed"]):0), 	//tcallowed
			   23   => ($formdata["chkinvopricechng"] <> ""?trim($formdata["chkinvopricechng"]):0), 	//allowinvoicepricechange
			   24   => trim($formdata["txtnamearb"]), 		//arbitemdescription
			   25   => trim($formdata["txtshortnamearb"]), 	//arbitemshortdescription
			   26   => (str_replace(',','',$formdata["hdntxtsalescostpcsprice"]) <> ""?trim(str_replace(',','',$formdata["hdntxtsalescostpcsprice"])):0), 		//costcaseprice
			   27   => (str_replace(',','',$formdata["hdntxtsalespcsprice"]) <> "" ?trim(str_replace(',','',$formdata["hdntxtsalespcsprice"])):0), 		//defaultcostprice			   
			...
			if($lastid[0][0]['result'] > 0)
			{
				...
				2        => (trim($formdata["txtprnt_seq_route"]) <>""?trim($formdata["txtprnt_seq_route"]):0), 	//PrintSequenceRoute
				3        => (trim($formdata["txtprnt_seq_cust"]) <> "" ?trim($formdata["txtprnt_seq_cust"]):0), 	//PrintSequenceCust				
				4        => ($formdata["chkfastmovingitem"] <> "" ?$formdata["chkfastmovingitem"]:0), 		//FastMovingItemFlag
				5        => ($formdata['ddlcode_day_form'] <> "" ?$formdata['ddlcode_day_form']:0), 		//CodeDateFormat
				6        => (trim($formdata["ddlitemtype"])<>""?trim($formdata["ddlitemtype"]):0), 		//ItemType
				...
				10       => ($formdata["ddlallowbatch"] <> ""?$formdata["ddlallowbatch"]:0), 		//AllowBatchEntry
				11	 	 => ($formdata['ddlitempkg'] <> ""?$formdata['ddlitempkg']:0),			//packagecode
