    ...
    */
            ...
            foreach($result_arr[1] as $row) {
               ...
               // $totalexpiryamount     += $row['totalexpiryamount'];
                ...
                $responce->rows[$i]['cell'] = array($row['routecode']." - ".$routename,$row['transactiondate'],
                                                    ...
                                                    $row['mop'],$row['salesamount'],$row['salescase'],$row['salespcs'],$row['goodreturnamount'],$row['returncase'],$row['returnpcs'],$row['totaldamagedamount'],$row['damagedcase'],$row['damagedpcs'],  /*$row['totalexpiryamount'],*/$row['freeamount'],$row['freecase'],$row['freepcs'],$row['invoiceamount'],$row['discountamount1'], $netamount,$row['immediatepaid'],$row['invoicebalance']
                                                    );
                $i++;
            }
        }
    ...
    public function exportAction()
    {
       ...
       //echo '<pre>',print_r($result_arr,1),'</pre>';exit;
        ...
        $data_arr["columns"] = array($this->translate->_('Route Code'),$this->translate->_( 'Transaction Date'),$this->translate->_('Transaction Time'),$this->translate->_('Invoice Number'),$this->translate->_('Salesman Code'),$this->translate->_('Customer Code'),$this->translate->_('Customer Name'),$this->translate->_('Payment Type'),$this->translate->_('Sales Amount'),$this->translate->_('Sales Qty Cases'),$this->translate->_('Sales Qty Pcs'),$this->translate->_('Good Return Amount'),$this->translate->_('Good Return Qty Cases'),$this->translate->_('Good Return Qty Pcs'),$this->translate->_('Bad Return Amount'),$this->translate->_('Bad Return Qty Cases'),$this->translate->_('Bad Return Qty Pcs'),/*$this->translate->_('Expiry Return Amount'),*/$this->translate->_('Free Amount'),$this->translate->_('Free Qty Cases'),$this->translate->_('Free Qty Pcs'),$this->translate->_('Invoice Amount'),$this->translate->_('Discount Amount'),$this->translate->_('Net Amount'),$this->translate->_('Immediate Paid'),$this->translate->_('Invoice Balance'));
        ...
        for($i = 0; $i < count($data); $i++)
        {
			...
			$netamount = $data[$i]['invoiceamount']-$data[$i]['discountamount1'];
            $column_model_arr[$data[$i]['routecode']." - ".$routename][$data[$i]['transactiondate']][] =
                                                array($data[$i]['transactiontime'],$data[$i]['invoicenumber'],$data[$i]['salesmancode'],$data[$i]['customercode'],$customername,
                                                    $data[$i]['mop'],$data[$i]['salesamount'],$data[$i]['salescase'],$data[$i]['salespcs'],$data[$i]['goodreturnamount'],$data[$i]['returncase'],$data[$i]['returnpcs'],$data[$i]['totaldamagedamount'],$data[$i]['damagedcase'],$data[$i]['damagedpcs'],/*$result_arr[0][$i]['totalexpiryamount'],*/$data[$i]['freeamount'],$data[$i]['freecase'],$data[$i]['freepcs'],$data[$i]['invoiceamount'],$data[$i]['discountamount1'],
                                                    $netamount,$data[$i]['immediatepaid'],$data[$i]['invoicebalance']);
        }
