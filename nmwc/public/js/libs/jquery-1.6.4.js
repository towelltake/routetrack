...
(function(window, undefined) {
			...
			function resolveFunc(i) {
				return function(value) {
					...
					if(!(--count )) {
						...
						// outside the $.when method. Cloning the object into a fresh array solves the
						// issue
	...
	jQuery.extend({
		...
		removeData : function(elem, name, pvt /* Internal Use Only */
			...
			if(jQuery.support.deleteExpando || !cache.setInterval) { delete
				cache[id];
			} else {
				cache[id] = null;
			}
			...
			if(internalCache) {
				...
				if(jQuery.support.deleteExpando) { delete
					elem[jQuery.expando];
	...
	jQuery.buildFragment = function(args, nodes, scripts) {
		...
		// Only cache "small" (1/2 KB) HTML strings that are associated with the main
		// document
	...
	jQuery.fn.extend({
		...
		animate : function(prop, speed, easing, callback) {
			...
			return this[ optall.queue === false ? "each" : "queue" ](function() {
				...
				// will store per property easing and be used to determine when an animation is
				// complete
				...
				for(p in prop ) {
					...
					if(p !== name) {
						prop[name] = prop[p]; delete
						prop[p];
					}
					val = prop[name];

					// easing resolution: per property > opt.specialEasing > opt.easing > 'swing'
					// (default)
	...
	jQuery.each(["Height", "Width"], function(i, name) {
		...
		jQuery.fn[type] = function(size) {
			...
			if(jQuery.isWindow(elem)) {
				// Everyone else use document.documentElement or document.body depending on
				// Quirks vs Standards mode
				// 3rd condition allows Nokia support, as it supports the docElem prop but not
				// CSS1Compat
