...
const pageTitle = computed(() =>
  isCreate.value ? `${t.create} ${t.customer_authorize_group}` : isView.value ? `${t.view} ${t.customer_authorize_group}` : `${t.edit} ${t.customer_authorize_group}`,
);
...
function submit() {
  ...
  if (isCreate.value) {
    form.post("/account/customer-authorize-group");
    return;
  }

  form.put(`/account/customer-authorize-group/${form.groupnumber}`);
}
...
<template>
  ...
  >
    <template #extra>
      <div class="d-flex gap-2">
        <button class="btn btn-alt-secondary" @click="router.get('/account/customer-authorize-group')">
          <i class="fa fa-arrow-left me-1"></i> {{ t.back }}
        </button>
        <button
          v-if="isView"
          class="btn btn-primary"
          @click="router.get(`/account/customer-authorize-group/${form.groupnumber}/edit`)"
        >
  ...
  <div class="content">
    <BaseBlock :title="t.authorize_group_details">
      ...
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="mb-0">{{ t.assigned_items }}</h5>
