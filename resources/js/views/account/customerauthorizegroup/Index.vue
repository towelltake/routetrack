...
function deleteRow() {
  router.delete(`/account/customer-authorize-group/${confirmingDelete.value.groupnumber}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.customer_authorize_group" />

  <BasePageHeading
    :title="t.customer_authorize_group"
    :subtitle="t.customer_authorize_group_note"
  >
    <template #extra>
      <button
        v-if="can('account customer authorize group', 'create')"
        class="btn btn-primary"
        @click="router.get('/account/customer-authorize-group/create')"
      >
  ...
  <div class="content">
    <BaseBlock :title="t.customer_authorize_group_overview">
      <template #options>
          ...
          <tbody>
            ...
            <tr v-for="(record, index) in rows" :key="record.groupnumber">
              ...
              <td class="text-center text-nowrap">
                <button
                  v-if="canViewAction('account customer authorize group')"
                  class="btn btn-sm btn-alt-info me-1"
                  :title="t.view"
                  @click="router.get(`/account/customer-authorize-group/${record.groupnumber}`)"
                >
                  <i class="fa fa-eye"></i>
                </button>
                <button
                  v-if="can('account customer authorize group', 'edit')"
                  class="btn btn-sm btn-alt-secondary me-1"
                  :title="t.edit"
                  @click="router.get(`/account/customer-authorize-group/${record.groupnumber}/edit`)"
                >
                  <i class="fa fa-pen"></i>
                </button>
                <button
                  v-if="can('account customer authorize group', 'delete')"
