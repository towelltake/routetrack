...
function deleteRow() {
  router.delete(`/account/salesman-message/${confirmingDelete.value.messagekey}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.salesman_message" />

  <BasePageHeading :title="t.salesman_message" :subtitle="t.salesman_message_note">
    <template #extra>
      <button
        v-if="can('salesman message', 'create')"
        class="btn btn-primary"
        @click="router.get('/account/salesman-message/create')"
      >
        <i class="fa fa-plus me-1"></i> {{ t.create_salesman_message }}
      </button>
    </template>
  </BasePageHeading>

  <div class="content">
    <BaseBlock :title="t.salesman_message_overview">
      <template #options>
          ...
          <tbody>
            ...
            <tr v-for="(record, index) in rows" :key="record.messagekey">
              ...
              <td class="text-center text-nowrap">
                <button
                  v-if="canViewAction('salesman message')"
                  class="btn btn-sm btn-alt-info me-1"
                  :title="t.view"
                  @click="router.get(`/account/salesman-message/${record.messagekey}`)"
                >
                  <i class="fa fa-eye"></i>
                </button>
                <button
                  v-if="can('salesman message', 'edit')"
                  class="btn btn-sm btn-alt-secondary me-1"
                  :title="t.edit"
                  @click="router.get(`/account/salesman-message/${record.messagekey}/edit`)"
                >
                  <i class="fa fa-pen"></i>
                </button>
                <button
                  v-if="can('salesman message', 'delete')"
      ...
      <div class="d-flex justify-content-between align-items-center pt-3">
        <div class="text-muted fs-sm">
          {{ t.showing }} {{ messages.from ?? 0 }} {{ t.to }} {{ messages.to ?? 0 }} {{ t.of }} {{ messages.total ?? 0 }}
        </div>
        <div class="btn-group">
          <button class="btn btn-sm btn-alt-secondary" :disabled="!messages.prev_page_url" @click="reloadList((messages.current_page || 1) - 1)">{{ t.previous }}</button>
          <button class="btn btn-sm btn-alt-secondary" disabled>{{ messages.current_page || 1 }} / {{ messages.last_page || 1 }}</button>
          <button class="btn btn-sm btn-alt-secondary" :disabled="!messages.next_page_url" @click="reloadList((messages.current_page || 1) + 1)">{{ t.next }}</button>
        </div>
      </div>
    </BaseBlock>
  </div>
  ...
  <div v-if="confirmingDelete" class="modal fade show d-block" style="background: rgba(0,0,0,.45)">
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">{{ t.delete_message_confirm }} <strong>{{ confirmingDelete.messagedescription }}</strong>?</div>
