...
const props = defineProps({
  ...
  creditNoteCustomerData: { type: Object, required: true },
...
const pageTitle = computed(() =>
  isView.value ? t.view_customer_credit_note : t.create_customer_credit_note,
);

const routeMeta = ref({
  salesmancode: props.initialMeta.salesmancode ?? "",
  salesmanname: props.initialMeta.salesmanname ?? "",
  documentnumber: props.initialMeta.documentnumber ?? props.creditNoteCustomerData.documentnumber ?? "",
...
const invoiceTotals = ref({
  invoice_total: props.creditNoteCustomerData.invoiceamount ?? 0,
  balance_total: props.creditNoteCustomerData.balanceamount ?? 0,
});

const form = useForm({
  date: props.filters.date,
  routecode: props.creditNoteCustomerData.routecode ?? "",
  salesmancode: props.creditNoteCustomerData.salesmancode ?? "",
  customercode: props.creditNoteCustomerData.customercode ?? "",
  paymentmode: Number(props.creditNoteCustomerData.paymentmode ?? 0),
  amount: props.creditNoteCustomerData.amount ?? "",
  invoice_total: props.creditNoteCustomerData.invoiceamount ?? 0,
  balance_total: props.creditNoteCustomerData.balanceamount ?? 0,
  remarks1: props.creditNoteCustomerData.remarks1 ?? "",
  remarks2: props.creditNoteCustomerData.remarks2 ?? "",
  checknumber: props.creditNoteCustomerData.checknumber ?? "",
  checkdate: props.creditNoteCustomerData.checkdate ?? "",
  bankcode: props.creditNoteCustomerData.bankcode ?? "",
  erpreferencenumber: props.creditNoteCustomerData.erpreferencenumber ?? "",
  firstoutstanding: !!props.creditNoteCustomerData.firstoutstanding,
  invoice_ids: [],
  invoice_amounts: [],
});
...
const routeLabel = computed(() => {
  if (isView.value) {
    return props.creditNoteCustomerData.routeLabel || "-";
  }
...
const customerLabel = computed(() => {
  if (isView.value) {
    return props.creditNoteCustomerData.customerLabel || "-";
  }
...
const salesmanLabel = computed(() => {
  if (isView.value) {
    return props.creditNoteCustomerData.salesmancode
      ? `${props.creditNoteCustomerData.salesmancode} - ${props.creditNoteCustomerData.salesmanname || "-"}`
      : "-";
  }
...
watch(
  ...
  (rows) => {
    ...
    form.amount = roundAmount(selectedBalanceTotal.value);
  },
...
watch(
  ...
  (value) => {
    ...
    invoices.value = invoices.value.map((row) => ({
      ...row,
      selected: Number(row.invoicebalance || 0) > 0,
      allocatedamount: Number(row.invoicebalance || 0) > 0 ? Number(row.invoicebalance || 0) : 0,
    }));
  },
);
...
async function loadRouteMeta(resetCustomer = true) {
  ...
  const { data } = await axios.get("/account/transaction/credit-note/customer/route-meta", {
...
async function loadInvoices() {
  ...
  const { data } = await axios.get("/account/transaction/credit-note/customer/invoices", {
    params: {
...
function submit() {
  ...
  form.post("/account/transaction/credit-note/customer");
}

function backToOverview() {
  router.get("/account/transaction/credit-note/customer", { date: props.filters.date });
}

function removeRecord() {
  if (!props.creditNoteCustomerData.transactionkey || !window.confirm(t.customer_credit_note_delete_confirm)) {
    return;
  }

  router.delete(`/account/transaction/credit-note/customer/${props.creditNoteCustomerData.transactionkey}`, {
...
<template>
  ...
  <div class="content">
    <BaseBlock :title="t.credit_note_customer_details">
      <div class="row g-4 mb-3">
        ...
        <div class="col-md-3">
          ...
          <input :value="isView ? creditNoteCustomerData.documentnumber : routeMeta.documentnumber" class="form-control" readonly />
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t.erp_reference_no }}</label>
          <input v-model="form.erpreferencenumber" class="form-control" :readonly="isView" />
          <div v-if="form.errors.erpreferencenumber" class="text-danger fs-sm mt-1">{{ form.errors.erpreferencenumber }}</div>
        </div>
        <div class="col-md-3 d-flex align-items-end">
          <div class="form-check mb-2">
            <input id="first-outstanding" v-model="form.firstoutstanding" class="form-check-input" type="checkbox" :disabled="isView" />
        ...
        <table class="table table-hover table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="text-end">{{ t.present_balance }}</th>
          ...
          <tbody>
            <tr v-if="!invoices.length">
              <td colspan="8" class="text-center text-muted py-4">{{ t.no_outstanding_invoices_found }}</td>
            </tr>
    ...
    <BaseBlock :title="t.mode_of_payment" class="mt-4">
      <div class="row g-4 mb-3">
        ...
        <div class="col-md-3">
          ...
          <input :value="formatNumber(form.amount)" class="form-control" readonly />
