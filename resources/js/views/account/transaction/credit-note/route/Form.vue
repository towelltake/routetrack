...
const props = defineProps({
  ...
  creditNoteRouteData: { type: Object, required: true },
...
const pageTitle = computed(() =>
  isView.value ? t.view_route_credit_note : t.create_route_credit_note,
);

const routeMeta = ref({
  salesmancode: props.initialMeta.salesmancode ?? "",
  salesmanname: props.initialMeta.salesmanname ?? "",
  documentnumber: props.initialMeta.documentnumber ?? props.creditNoteRouteData.documentnumber ?? "",
  invoicenumber: props.initialMeta.invoicenumber ?? props.creditNoteRouteData.invoicenumber ?? "",
});

const form = useForm({
  date: props.filters.date,
  routecode: props.creditNoteRouteData.routecode ?? "",
  salesmancode: props.creditNoteRouteData.salesmancode ?? "",
  amount: props.creditNoteRouteData.amount ?? "",
  remarks1: props.creditNoteRouteData.remarks1 ?? "",
  remarks2: props.creditNoteRouteData.remarks2 ?? "",
  erpreferencenumber: props.creditNoteRouteData.erpreferencenumber ?? "",
});

const routeLabel = computed(() => {
  if (isView.value) {
    return props.creditNoteRouteData.routeLabel || "-";
  }
...
async function loadRouteMeta() {
  ...
  const { data } = await axios.get("/account/transaction/credit-note/route/route-meta", {
...
function submit() {
  ...
  form.post("/account/transaction/credit-note/route");
}

function backToOverview() {
  router.get("/account/transaction/credit-note/route", { date: props.filters.date });
}

function removeRecord() {
  if (!props.creditNoteRouteData.transactionkey || !window.confirm(t.route_credit_note_delete_confirm)) {
    return;
  }

  router.delete(`/account/transaction/credit-note/route/${props.creditNoteRouteData.transactionkey}`, {
...
<template>
  ...
  <div class="content">
    <BaseBlock :title="t.credit_note_route_details">
      <div class="row g-4 mb-3">
        ...
        <div class="col-md-3">
          ...
          <input :value="isView ? creditNoteRouteData.documentnumber : routeMeta.documentnumber" class="form-control" readonly />
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t.invoice_number }}</label>
          <input :value="isView ? creditNoteRouteData.invoicenumber : routeMeta.invoicenumber" class="form-control" readonly />
        </div>
        ...
        <div class="col-md-4">
          ...
          <input
            :value="isView ? `${creditNoteRouteData.salesmancode} - ${creditNoteRouteData.salesmanname || '-'}` : routeMeta.salesmanname ? `${routeMeta.salesmancode} - ${routeMeta.salesmanname}` : ''"
            class="form-control"
            readonly
          />
