...
function loadForDate(event) {
  router.get("/account/transaction/debit-note/customer", { date: event.target.value });
}

function openCreate() {
  router.get("/account/transaction/debit-note/customer/create", { date: props.filters.date });
}

function openView(transactionkey) {
  router.get(`/account/transaction/debit-note/customer/${transactionkey}`, { date: props.filters.date });
}

function removeRecord(transactionkey) {
  if (!window.confirm(t.customer_debit_note_delete_confirm)) {
    return;
  }

  router.delete(`/account/transaction/debit-note/customer/${transactionkey}`, {
...
<template>
  <Head :title="t.debit_note_customer" />

  <BasePageHeading
    :title="t.debit_note_customer"
    :subtitle="t.debit_note_customer_note"
  >
    <template #extra>
      <button v-if="canCreate" class="btn btn-primary" @click="openCreate">
        <i class="fa fa-plus me-1"></i> {{ t.add_customer_debit_note }}
      </button>
    </template>
  </BasePageHeading>
