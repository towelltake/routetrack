...
function loadForDate(event) {
  router.get("/account/transaction/debit-note/route", { date: event.target.value });
}

function openCreate() {
  router.get("/account/transaction/debit-note/route/create", { date: props.filters.date });
}

function openView(transactionkey) {
  router.get(`/account/transaction/debit-note/route/${transactionkey}`, { date: props.filters.date });
}

function removeRecord(transactionkey) {
  if (!window.confirm(t.route_debit_note_delete_confirm)) {
    return;
  }

  router.delete(`/account/transaction/debit-note/route/${transactionkey}`, {
...
<template>
  <Head :title="t.debit_note_route" />

  <BasePageHeading
    :title="t.debit_note_route"
    :subtitle="t.debit_note_route_note"
  >
    <template #extra>
      <button v-if="canCreate" class="btn btn-primary" @click="openCreate">
        <i class="fa fa-plus me-1"></i> {{ t.add_route_debit_note }}
      </button>
    </template>
  </BasePageHeading>
