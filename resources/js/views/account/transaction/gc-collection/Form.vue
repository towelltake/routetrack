...
const props = defineProps({
  ...
  gcCollectionData: { type: Object, required: true },
...
const canDelete = computed(
  () => !!(permission.value.all || permission.value.delete),
);
const isView = computed(() => props.mode === "view");
const pageTitle = computed(() =>
  isView.value ? t.view_gc_collection : t.create_gc_collection,
);
...
const routeMeta = ref({
  ...
  documentnumber:
    props.initialMeta.documentnumber ??
    props.gcCollectionData.documentnumber ??
    "",
...
const invoiceTotals = ref({
  invoice_total: props.gcCollectionData.invoiceamount ?? 0,
  balance_total: props.gcCollectionData.balanceamount ?? 0,
});

const form = useForm({
  date: props.filters.date,
  routecode: props.gcCollectionData.routecode ?? "",
  salesmancode: props.gcCollectionData.salesmancode ?? "",
  customercode: props.gcCollectionData.customercode ?? "",
  paymentmode: Number(props.gcCollectionData.paymentmode ?? 0),
  amount: props.gcCollectionData.amount ?? "",
  invoice_total: props.gcCollectionData.invoiceamount ?? 0,
  balance_total: props.gcCollectionData.balanceamount ?? 0,
  erpreferencenumber: props.gcCollectionData.erpreferencenumber ?? "",
  checknumber: props.gcCollectionData.checknumber ?? "",
  checkdate: props.gcCollectionData.checkdate ?? "",
  bankcode: props.gcCollectionData.bankcode ?? "",
  firstoutstanding: !!props.gcCollectionData.firstoutstanding,
  invoice_ids: [],
  invoice_amounts: [],
});
...
const routeLabel = computed(() => {
  if (isView.value) {
    return props.gcCollectionData.routeLabel || "-";
  }

  return (
    props.routeOptions.find(
      (option) => String(option.id) === String(form.routecode),
    )?.label || "-"
  );
});
...
const customerLabel = computed(() => {
  if (isView.value) {
    return props.gcCollectionData.customerLabel || "-";
  }

  return (
    routeMeta.value.customerOptions.find(
...
const salesmanLabel = computed(() => {
  if (isView.value) {
    return props.gcCollectionData.salesmancode
      ? `${props.gcCollectionData.salesmancode} - ${props.gcCollectionData.salesmanname || "-"}`
      : "-";
  }
...
const selectedInvoices = computed(() =>
  invoices.value.filter((row) => row.selected),
);
...
async function loadRouteMeta(resetCustomer = true) {
  ...
  const { data } = await axios.get(
    "/account/transaction/gc-collection/route-meta",
    {
...
async function loadInvoices() {
  ...
  const { data } = await axios.get(
    "/account/transaction/gc-collection/invoices",
    {
      params: {
...
function submit() {
  ...
  form.post("/account/transaction/gc-collection");
}

function backToOverview() {
  router.get("/account/transaction/gc-collection", {
    date: props.filters.date,
  });
}
...
function removeRecord() {
  if (
    !props.gcCollectionData.transactionkey ||
    !window.confirm(t.gc_collection_delete_confirm)
  ) {
    return;
  }

  router.delete(
    `/account/transaction/gc-collection/${props.gcCollectionData.transactionkey}`,
    {
...
<template>
  ...
  <div class="content">
    <BaseBlock :title="t.gc_collection_details">
      <div class="row g-4 mb-3">
        <div class="col-md-3">
          ...
          <input
            ...
            :readonly="isView"
          />
          <div v-if="form.errors.date" class="text-danger fs-sm mt-1">
            {{ form.errors.date }}
          </div>
        </div>
        <div class="col-md-3">
          ...
          <input
            :value="
              isView
                ? gcCollectionData.documentnumber
                : routeMeta.documentnumber
            "
            class="form-control"
            readonly
          />
        ...
        <div class="col-md-3">
          ...
          <input
            v-model="form.erpreferencenumber"
            class="form-control"
            :readonly="isView"
          />
          <div
            v-if="form.errors.erpreferencenumber"
        ...
        <div class="col-md-3 d-flex align-items-end">
          <div class="form-check mb-2">
            <input
              ...
              :disabled="isView"
            />
            <label class="form-check-label" for="first-outstanding"
              >{{ t.first_outstanding }}</label
            >
          </div>
        </div>
        ...
        <div class="col-md-4">
          <label class="form-label"
            >{{ t.route }} <span v-if="!isView" class="text-danger">*</span></label
          >
          <select
            v-if="!isView"
        ...
        <div class="col-md-4">
          <label class="form-label"
            >{{ t.customer }} <span v-if="!isView" class="text-danger">*</span></label
          >
          <select
            v-if="!isView"
          ...
          <div v-if="form.errors.customercode" class="text-danger fs-sm mt-1">
    ...
    <BaseBlock :title="t.outstanding_invoices" class="mt-4">
      <div class="row g-3 mb-3">
        <div class="col-md-3">
          ...
          <input
            :value="formatNumber(form.invoice_total)"
            class="form-control"
            readonly
          />
        </div>
        <div class="col-md-3">
          ...
          <input
            :value="formatNumber(form.balance_total)"
            class="form-control"
            readonly
          />
        </div>
        <div class="col-md-3">
          ...
          <input
            :value="formatNumber(invoiceTotals.invoice_total)"
            class="form-control"
            readonly
          />
        </div>
        <div class="col-md-3">
          ...
          <input
            :value="formatNumber(invoiceTotals.balance_total)"
            class="form-control"
            readonly
          />
        </div>
          ...
          <tbody>
            <tr v-if="!invoices.length">
              <td colspan="8" class="text-center text-muted py-4">
                {{ t.no_outstanding_invoices_found }}
              </td>
            </tr>
      ...
      <div v-if="form.errors.invoice_ids" class="text-danger fs-sm mt-2">
        {{ form.errors.invoice_ids }}
      </div>
    </BaseBlock>
    ...
    <BaseBlock :title="t.mode_of_payment" class="mt-4">
      <div class="row g-4 mb-3">
        <div class="col-md-3">
          <label class="form-label"
            >{{ t.payment_mode }}
            <span v-if="!isView" class="text-danger">*</span></label
          >
          <select
          ...
          <div v-if="form.errors.paymentmode" class="text-danger fs-sm mt-1">
            {{ form.errors.paymentmode }}
          </div>
        </div>
        <div class="col-md-3">
          <label class="form-label"
          ...
          <input
            ...
            :readonly="isView"
          />
          <div v-if="form.errors.amount" class="text-danger fs-sm mt-1">
            {{ form.errors.amount }}
          </div>
        </div>
        <div class="col-md-3">
          ...
          <input
            ...
            class="form-control"
            :readonly="isView || !isChequeMode"
          />
          <div v-if="form.errors.checknumber" class="text-danger fs-sm mt-1">
            {{ form.errors.checknumber }}
          </div>
        </div>
        <div class="col-md-3">
          ...
          <input
            ...
            :readonly="isView || !isChequeMode"
          />
          <div v-if="form.errors.checkdate" class="text-danger fs-sm mt-1">
            {{ form.errors.checkdate }}
          </div>
        </div>
        <div class="col-md-4">
          ...
          <div v-if="form.errors.bankcode" class="text-danger fs-sm mt-1">
            {{ form.errors.bankcode }}
          </div>
        </div>
      </div>
    </BaseBlock>
    ...
    <BaseBlock
      v-if="isView && paymentDetails"
      :title="t.saved_payment_details"
      class="mt-4"
    >
      <div class="row g-4">
        <div class="col-md-3">
          ...
          <input
            :value="paymentDetails.typecode === 1 ? t.cheque : t.cash"
            class="form-control"
            readonly
          />
        </div>
        <div class="col-md-3">
          ...
          <input
            :value="formatNumber(paymentDetails.amount)"
            class="form-control"
            readonly
          />
        </div>
