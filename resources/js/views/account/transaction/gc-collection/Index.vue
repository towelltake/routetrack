...
function loadForDate(event) {
  router.get("/account/transaction/gc-collection", { date: event.target.value });
}

function openCreate() {
  router.get("/account/transaction/gc-collection/create", { date: props.filters.date });
}

function openView(transactionkey) {
  router.get(`/account/transaction/gc-collection/${transactionkey}`, { date: props.filters.date });
}

function removeRecord(transactionkey) {
  if (!window.confirm(t.gc_collection_delete_confirm)) {
    return;
  }

  router.delete(`/account/transaction/gc-collection/${transactionkey}`, {
...
<template>
  <Head :title="t.gc_collection" />

  <BasePageHeading
    :title="t.gc_collection"
    :subtitle="t.gc_collection_note"
  >
    <template #extra>
      <button v-if="canCreate" class="btn btn-primary" @click="openCreate">
        <i class="fa fa-plus me-1"></i> {{ t.add_gc_collection }}
      </button>
    </template>
  </BasePageHeading>
