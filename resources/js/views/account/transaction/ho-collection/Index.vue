...
function loadForDate(event) {
  router.get("/account/transaction/ho-collection", { date: event.target.value });
}

function openCreate() {
  router.get("/account/transaction/ho-collection/create", { date: props.filters.date });
}

function openView(transactionkey) {
  router.get(`/account/transaction/ho-collection/${transactionkey}`, { date: props.filters.date });
}

function removeRecord(transactionkey) {
  if (!window.confirm(t.ho_collection_delete_confirm)) {
    return;
  }

  router.delete(`/account/transaction/ho-collection/${transactionkey}`, {
...
<template>
  <Head :title="t.ho_collection" />

  <BasePageHeading
    :title="t.ho_collection"
    :subtitle="t.ho_collection_note"
  >
    <template #extra>
      <button v-if="canCreate" class="btn btn-primary" @click="openCreate">
        <i class="fa fa-plus me-1"></i> {{ t.add_ho_collection }}
      </button>
    </template>
  </BasePageHeading>
