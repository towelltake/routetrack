...
const resetCooldownKey = "forgot-password-resend-available-at";
...
const syncCountdown = () => {
  const availableAt = Number(window.sessionStorage.getItem(resetCooldownKey));
  ...
  if (remaining === 0) {
    window.sessionStorage.removeItem(resetCooldownKey);
    clearCountdown();
  }
};
...
const startCooldown = () => {
  ...
  window.sessionStorage.setItem(resetCooldownKey, String(availableAt));
  syncCountdown();

  clearCountdown();
  countdownInterval = window.setInterval(syncCountdown, 1000);
};

const isCooldownActive = computed(() => secondsRemaining.value > 0);
const isSubmitDisabled = computed(
  () => form.processing || isCooldownActive.value,
);
const submitLabel = computed(() => {
  if (isCooldownActive.value) {
    return (
      t.value.password_reset_resend_in?.replace(
        ":seconds",
        String(secondsRemaining.value),
      ) ?? `Resend in ${secondsRemaining.value}s`
    );
  }

  if (props.status) {
    return t.value.resend_password_reset_link ?? "Resend password reset link";
  }

  return t.value.email_password_reset_link ?? "Email password reset link";
});
...
<template>
  <Head :title="t.forgot_password_heading ?? 'Forgot Password'" />
        ...
        <div class="p-4 p-xl-5 flex-grow-1 d-flex align-items-center">
          <div class="w-100 login-panel-content">
            ...
            <h2 class="login-panel-title text-white mt-4 mb-3">
              {{
                t.forgot_password_panel_title ??
                "Restore secure access to route operations, collections, and reporting."
              }}
            </h2>
            <p class="login-panel-copy text-white-75 mb-4">
              {{
                t.forgot_password_panel_copy ??
                "Submit the registered email address for your TRAC account to receive a password reset link and regain access to operational workflows securely."
              }}
            </p>
          </div>
        </div>
            ...
            <div class="text-center mb-5">
              ...
              <p class="fw-medium text-muted">
                {{
                  ...
                  "Enter the registered email for your TRAC account. A password reset link will be sent so you can restore access to operational workflows securely."
                }}
              </p>
            </div>
