...
import CreateCashDescription from "./Create.vue";
import EditCashDescription from "./Edit.vue";
import ViewCashDescription from "./View.vue";
...
function submitCreate() {
  form.post("/basic/cashdescription", {
    preserveScroll: true,
...
function submitEdit() {
  form.put(`/basic/cashdescription/${editingId.value}`, {
    preserveScroll: true,
    onSuccess: closeModal,
  });
}
...
function deleteRow() {
  router.delete(`/basic/cashdescription/${confirmingDelete.value.code}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.cash_description" />

  <BasePageHeading :title="t.cash_description" :subtitle="t.cash_description_note">
    <template #extra>
      <button
        v-if="can('cash description', 'create')"
        class="btn btn-primary"
        @click="openCreate"
      >
        <i class="fa fa-plus me-1"></i> {{ t.add_cash_description }}
      </button>
    </template>
  </BasePageHeading>

  <div class="content">
    <BaseBlock :title="t.cash_description_list">
      <template #options>
        ...
        <table class="table table-hover table-vcenter fs-sm">
          ...
          <tbody>
            ...
            <tr v-for="(record, index) in rows" :key="record.code">
              <td class="text-muted">{{ (items.from ?? 1) + index }}</td>
              <td>{{ record.code }}</td>
              <td class="fw-semibold">{{ record.description }}</td>
              <td>{{ record.arbdescription }}</td>
              <td class="text-center text-nowrap">
                <button
                  v-if="canViewAction('cash description')"
                ...
                <button
                  v-if="can('cash description', 'edit')"
                ...
                <button
                  v-if="can('cash description', 'delete')"
      ...
      <div class="d-flex justify-content-between align-items-center pt-3">
        ...
        <div class="btn-group">
          <button
            ...
            @click="reloadList((items.current_page || 1) - 1)"
          >
            {{ t.previous }}
          </button>
          <button class="btn btn-sm btn-alt-secondary" disabled>
            {{ items.current_page || 1 }} / {{ items.last_page || 1 }}
          </button>
          <button
            class="btn btn-sm btn-alt-secondary"
  ...
  >
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">{{ t.delete_cash_description_label }} <strong>{{ confirmingDelete.description }}</strong>?</div>
