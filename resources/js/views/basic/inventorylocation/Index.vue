...
import CreateInventoryLocation from "./Create.vue";
import EditInventoryLocation from "./Edit.vue";
import ViewInventoryLocation from "./View.vue";
...
const rows = computed(() => props.locations?.data ?? []);
...
function submitCreate() {
  form.post("/basic/inventorylocation", {
    preserveScroll: true,
...
function submitEdit() {
  form.put(`/basic/inventorylocation/${editingId.value}`, {
    preserveScroll: true,
    onSuccess: closeModal,
  });
}
...
function deleteRow() {
  router.delete(`/basic/inventorylocation/${confirmingDelete.value.code}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.inventory_location" />

  <BasePageHeading :title="t.inventory_location" :subtitle="t.inventory_location_note">
    <template #extra>
      <button
        v-if="can('inventory location', 'create')"
        class="btn btn-primary"
        @click="openCreate"
      >
        <i class="fa fa-plus me-1"></i>
        {{ t.add_inventory_location }}
      </button>
    </template>
  </BasePageHeading>

  <div class="content">
    <BaseBlock :title="t.inventory_location_list">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-hover table-vcenter fs-sm">
          ...
          <tbody>
            ...
            <tr v-for="(record, index) in rows" :key="record.code">
              <td class="text-muted">{{ (locations.from ?? 1) + index }}</td>
              <td>{{ record.code }}</td>
              <td class="fw-semibold">{{ record.description }}</td>
              <td>{{ record.arbdescription }}</td>
              <td class="text-center text-nowrap">
                <button
                  v-if="canViewAction('inventory location')"
                ...
                <button
                  v-if="can('inventory location', 'edit')"
                ...
                <button
                  v-if="can('inventory location', 'delete')"
      ...
      <div class="d-flex justify-content-between align-items-center pt-3">
        <div class="text-muted fs-sm">
          {{ t.showing }} {{ locations.from ?? 0 }} {{ t.to }} {{ locations.to ?? 0 }} {{ t.of }} {{ locations.total ?? 0 }}
        </div>
        <div class="btn-group">
          <button
            class="btn btn-sm btn-alt-secondary"
            :disabled="!locations.prev_page_url"
            @click="reloadList((locations.current_page || 1) - 1)"
          >
            {{ t.previous }}
          </button>
          <button class="btn btn-sm btn-alt-secondary" disabled>
            {{ locations.current_page || 1 }} / {{ locations.last_page || 1 }}
          </button>
          <button
            class="btn btn-sm btn-alt-secondary"
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">
          {{ t.delete_inventory_location_label }} <strong>{{ confirmingDelete.description }}</strong>?
        </div>
