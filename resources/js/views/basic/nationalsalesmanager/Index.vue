...
import CreateNationalSalesManager from "./Create.vue";
import EditNationalSalesManager from "./Edit.vue";
import ViewNationalSalesManager from "./View.vue";
...
function fillForm(record) {
  ...
  form.nationalsalesmanagername = record.nationalsalesmanagername ?? "";
  form.arbnationalsalesmanagername = record.arbnationalsalesmanagername ?? "";
...
function openView(record) {
  editingId.value = record.nationalsalesmanagercode;
...
function submitCreate() {
  form.post("/basic/nationalsalesmanager", {
    preserveScroll: true,
...
function submitEdit() {
  form.put(`/basic/nationalsalesmanager/${editingId.value}`, {
    preserveScroll: true,
    onSuccess: closeModal,
  });
}
...
function deleteRow() {
  router.delete(`/basic/nationalsalesmanager/${confirmingDelete.value.nationalsalesmanagercode}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.national_sales_manager" />

  <BasePageHeading :title="t.national_sales_manager" :subtitle="t.national_sales_manager_note">
    <template #extra>
      <button
        v-if="can('national sales mgr', 'create')"
        class="btn btn-primary"
        @click="openCreate"
      >
        <i class="fa fa-plus me-1"></i>
        {{ t.add_national_sales_manager }}
      </button>
    </template>
  </BasePageHeading>

  <div class="content">
    <BaseBlock :title="t.national_sales_manager_list">
      <template #options>
        ...
        <table class="table table-hover table-vcenter fs-sm">
          <thead>
            <tr>
              <th>#</th>
              <th>{{ t.code }}</th>
              <th>{{ t.national_sales_manager }}</th>
          ...
          <tbody>
            ...
            <tr v-for="(record, index) in rows" :key="record.nationalsalesmanagercode">
              <td class="text-muted">{{ (managers.from ?? 1) + index }}</td>
              <td>{{ record.nationalsalesmanagercode }}</td>
              <td class="fw-semibold">{{ record.nationalsalesmanagername }}</td>
              ...
              <td class="text-center text-nowrap">
                <button
                  v-if="canViewAction('national sales mgr')"
                ...
                <button
                  v-if="can('national sales mgr', 'edit')"
                ...
                <button
                  v-if="can('national sales mgr', 'delete')"
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">
          {{ t.delete_national_sales_manager_label }} <strong>{{ confirmingDelete.nationalsalesmanagername }}</strong>?
        </div>
