...
import CreateRegionManager from "./Create.vue";
import EditRegionManager from "./Edit.vue";
import ViewRegionManager from "./View.vue";
...
function fillForm(record) {
  ...
  form.regionmanagername = record.regionmanagername ?? "";
  form.arbregionmanagername = record.arbregionmanagername ?? "";
...
function openView(record) {
  editingId.value = record.regionmanagercode;
...
function submitEdit() {
  form.put(`/basic/regionmanager/${editingId.value}`, {
    preserveScroll: true,
    onSuccess: closeModal,
  });
}
...
function deleteRow() {
  router.delete(`/basic/regionmanager/${confirmingDelete.value.regionmanagercode}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.regional_manager" />

  <BasePageHeading :title="t.regional_manager" :subtitle="t.regional_manager_note">
    <template #extra>
      <button
        v-if="can('region manager', 'create')"
        class="btn btn-primary"
        @click="openCreate"
      >
        <i class="fa fa-plus me-1"></i>
        {{ t.add_regional_manager }}
      </button>
    </template>
  </BasePageHeading>

  <div class="content">
    <BaseBlock :title="t.regional_manager_list">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-hover table-vcenter fs-sm">
          ...
          <tbody>
            ...
            <tr v-for="(record, index) in rows" :key="record.regionmanagercode">
              <td class="text-muted">{{ (managers.from ?? 1) + index }}</td>
              <td>{{ record.regionmanagercode }}</td>
              <td class="fw-semibold">{{ record.regionmanagername }}</td>
              ...
              <td class="text-center text-nowrap">
                <button
                  v-if="canViewAction('region manager')"
                ...
                <button
                  v-if="can('region manager', 'delete')"
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">
          {{ t.delete_regional_manager_label }} <strong>{{ confirmingDelete.regionmanagername }}</strong>?
        </div>
