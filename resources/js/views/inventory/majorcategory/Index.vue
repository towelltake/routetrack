...
const props = defineProps({
  majorCategories: { type: Object, required: true },
  filters: { type: Object, default: () => ({}) },
  companyGroups: { type: Array, default: () => [] },
});
...
const rows = computed(() => props.majorCategories?.data ?? []);
const search = ref(props.filters?.search ?? "");
const perPage = ref(props.filters?.per_page ?? 10);
const sortBy = ref(props.filters?.sort_by ?? "majorcategorycode");
...
function reloadList(pageNumber = 1) {
  router.get("/inventory/majorcategory", {
    ...
    preserveState: true,
    replace: true,
    only: ["majorCategories", "filters", "companyGroups"],
  });
}
...
function submit() {
  ...
  if (isEditing.value) {
    form.put(`/inventory/majorcategory/${editingId.value}`, options);
    return;
  }

  form.post("/inventory/majorcategory", options);
}

function deleteRow() {
  router.delete(`/inventory/majorcategory/${confirmingDelete.value.majorcategorycode}`, {
    preserveScroll: true,
    onSuccess: () => {
...
function companyGroupLabel(record) {
  return locale === "ar"
    ? (record.company_group_name_ar || record.company_group_name || "")
    : (record.company_group_name || record.company_group_name_ar || "");
}
...
<template>
  <Head :title="t.major_category" />

  <BasePageHeading :title="t.major_category" :subtitle="t.major_category_note">
    <template #extra>
      <button v-if="can('major category', 'create')" class="btn btn-primary" @click="openAdd">
        <i class="fa fa-plus me-1"></i> {{ t.add_major_category }}
      </button>
    </template>
  </BasePageHeading>

  <div class="content">
    <BaseBlock :title="t.major_category_list">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-hover table-vcenter fs-sm">
          ...
          <tbody>
            ...
            <tr v-for="record in rows" :key="record.majorcategorycode">
              <td class="fw-semibold">{{ record.alternatecode || "-" }}</td>
              <td>{{ record.description }}</td>
              <td>{{ record.arbdescription || "-" }}</td>
              <td>{{ companyGroupLabel(record) }}</td>
              ...
              <td class="text-center text-nowrap">
                <button
                  v-if="canViewAction('major category')"
                ...
                <button
                  v-if="can('major category', 'delete')"
      ...
      <div class="d-flex justify-content-between align-items-center pt-3">
        <div class="text-muted fs-sm">
          {{ t.showing }} {{ majorCategories.from ?? 0 }} {{ t.to }} {{ majorCategories.to ?? 0 }} {{ t.of }} {{ majorCategories.total ?? 0 }}
        </div>
        <div class="btn-group">
          <button class="btn btn-sm btn-alt-secondary" :disabled="!majorCategories.prev_page_url" @click="reloadList((majorCategories.current_page || 1) - 1)">{{ t.previous }}</button>
          <button class="btn btn-sm btn-alt-secondary" disabled>{{ majorCategories.current_page || 1 }} / {{ majorCategories.last_page || 1 }}</button>
          <button class="btn btn-sm btn-alt-secondary" :disabled="!majorCategories.next_page_url" @click="reloadList((majorCategories.current_page || 1) + 1)">{{ t.next }}</button>
        </div>
      </div>
    </BaseBlock>
  </div>
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">
            {{ isViewing ? t.major_category : (isEditing ? t.edit_major_category : t.create_major_category) }}
          </h5>
        ...
        <form @submit.prevent="submit">
          <div class="modal-body">
            ...
            <div class="mb-3">
              <label class="form-label">{{ t.company_group }} <span class="text-danger">*</span></label>
              <select v-model="form.companygroupcode" class="form-select" :disabled="isViewing">
                <option value="">{{ t.select_company_group }}</option>
                <option
                  v-for="group in companyGroups"
                  :key="group.companygroupcode"
                  :value="group.companygroupcode"
                >
                  {{ locale === "ar" ? (group.arbdescription || group.description) : (group.description || group.arbdescription) }}
                </option>
              </select>
              <div v-if="form.errors.companygroupcode" class="text-danger fs-sm mt-1">
                {{ form.errors.companygroupcode }}
              </div>
            </div>
  ...
  >
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">
          {{ t.major_category_delete_confirm }}
        </div>
        <div class="modal-footer">
