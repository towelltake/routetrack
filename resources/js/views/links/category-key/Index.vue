...
async function loadData() {
  ...
  if (!form.value.category_id || form.value.route_id === "") {
    errorMessage.value = t.category_key_and_route_required;
    return;
  }
...
<template>
  <Head :title="t.category_key" />

  <BasePageHeading :title="t.category_key" :subtitle="t.category_key_note">
    <template #extra>
  ...
  <div class="content">
    <div v-if="!available" class="alert alert-warning d-flex align-items-center" role="alert">
      ...
      <div>{{ t.legacy_category_key_link_required }}</div>
    </div>
    ...
    <BaseBlock :title="t.category_key">
      <div class="row g-3 align-items-end">
        <div class="col-md-5">
          <label class="form-label">{{ t.category_key }}</label>
