...
const availableItems = computed(() => {
  ...
  return allItems.value.filter((item) => !selectedIds.has(item.id));
});
...
function saveLinks() {
  ...
  if (form.value.target_item_group === "") {
    errorMessage.value = t.to_item_group_required;
    return;
  }
  ...
  router.post(
    props.formMeta.saveUrl,
    {
      ...
      target_item_group: form.value.target_item_group,
...
<template>
  <Head :title="t.items_group" />

  <BasePageHeading :title="t.items_group" :subtitle="t.items_group_note">
    <template #extra>
  ...
  <div class="content">
    <div v-if="!available" class="alert alert-warning d-flex align-items-center" role="alert">
      ...
      <div>{{ t.legacy_items_group_link_required }}</div>
    </div>
    ...
    <BaseBlock :title="t.items_group">
      <div class="row g-3 align-items-end">
        ...
        <div class="col-lg-4">
          <label class="form-label">{{ t.to_item_group }}</label>
          <select v-model="form.target_item_group" class="form-select" :disabled="!available || loading">
            <option value="">{{ t.select_placeholder }}</option>
            <option v-for="option in itemGroupOptions" :key="`target-${option.id}`" :value="option.id">
              {{ option.label }}
            </option>
          </select>
        </div>
      ...
      <div class="row g-3 pt-4 mb-3">
        ...
        <div class="col-lg-2">
          <div class="d-grid gap-2 h-100 align-content-center pt-lg-4">
            ...
            <button class="btn btn-alt-secondary" :disabled="loading || !selectedItems.length" @click="moveSelectedToLeft">
              &lt;
            </button>
            <button class="btn btn-alt-secondary" :disabled="loading || !selectedItems.length" @click="moveAllToLeft">
              &lt;&lt;
            </button>
          </div>
        </div>
        ...
        <div class="col-lg-5">
          <label class="form-label">{{ t.selected_item }}</label>
          <select
