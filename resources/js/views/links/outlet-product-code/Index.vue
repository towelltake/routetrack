...
const outletProductCodeOptions = computed(() => props.optionSets?.outletProductCodeOptions ?? []);
...
function validateForm() {
  if (!form.value.outlet_product_code) {
    return t.outlet_product_code_required;
  }
...
async function loadData() {
  ...
  try {
    const params = new URLSearchParams({
      outlet_product_code: String(form.value.outlet_product_code),
...
function saveLinks() {
  ...
  router.post(
    ...
    {
      outlet_product_code: form.value.outlet_product_code,
...
<template>
  <Head :title="t.outlet_product_code" />

  <BasePageHeading :title="t.outlet_product_code" :subtitle="t.outlet_product_code_note">
    <template #extra>
  ...
  <div class="content">
    <div v-if="!available" class="alert alert-warning d-flex align-items-center" role="alert">
      ...
      <div>
        {{ t.legacy_outlet_product_code_link_required }}
      </div>
    </div>
    ...
    <BaseBlock :title="t.outlet_product_code">
      <div class="row g-3 align-items-end">
        <div class="col-lg-4">
          <label class="form-label">{{ t.outlet_product_code }}</label>
          <select v-model="form.outlet_product_code" class="form-select" :disabled="!available || loading">
            <option value="">{{ t.select_placeholder }}</option>
            <option v-for="option in outletProductCodeOptions" :key="option.id" :value="option.id">{{ option.label }}</option>
          </select>
        </div>
