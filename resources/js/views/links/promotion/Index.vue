...
const promotionKeyOptions = computed(() => props.optionSets?.promotionKeyOptions ?? []);
...
function validateForm() {
  if (!form.value.promotion_key) {
    return t.promotion_key_required;
  }
...
async function loadData() {
  ...
  try {
    const params = new URLSearchParams({
      promotion_key: String(form.value.promotion_key),
...
function saveLinks() {
  ...
  router.post(
    ...
    {
      promotion_key: form.value.promotion_key,
...
<template>
  ...
  <BasePageHeading :title="t.promotion" :subtitle="t.promotion_link_note">
    <template #extra>
  ...
  <div class="content">
    <div v-if="!available" class="alert alert-warning d-flex align-items-center" role="alert">
      ...
      <div>
        {{ t.legacy_promotion_link_required }}
      </div>
    </div>
    ...
    <BaseBlock :title="t.promotion">
      <div class="row g-3 align-items-end">
        <div class="col-lg-4">
          <label class="form-label">{{ t.promotion_key }}</label>
          <select v-model="form.promotion_key" class="form-select" :disabled="!available || loading">
            <option value="">{{ t.select_placeholder }}</option>
            <option v-for="option in promotionKeyOptions" :key="option.id" :value="option.id">{{ option.label }}</option>
          </select>
        </div>
