...
const pricingKeyOptions = computed(() => props.optionSets?.pricingKeyOptions ?? []);
...
function validateForm() {
  if (!form.value.pricing_key) {
    return t.pricing_key_required;
  }
...
async function loadData() {
  ...
  try {
    const params = new URLSearchParams({
      pricing_key: String(form.value.pricing_key),
...
function saveLinks() {
  ...
  router.post(
    ...
    {
      pricing_key: form.value.pricing_key,
...
<template>
  <Head :title="t.special_price" />

  <BasePageHeading :title="t.special_price" :subtitle="t.special_price_link_note">
    <template #extra>
  ...
  <div class="content">
    <div v-if="!available" class="alert alert-warning d-flex align-items-center" role="alert">
      ...
      <div>
        {{ t.legacy_special_price_link_required }}
      </div>
    </div>
    ...
    <BaseBlock :title="t.special_price">
      <div class="row g-3 align-items-end">
        <div class="col-lg-4">
          <label class="form-label">{{ t.pricing_key }}</label>
          <select v-model="form.pricing_key" class="form-select" :disabled="!available || loading">
            <option value="">{{ t.select_placeholder }}</option>
            <option v-for="option in pricingKeyOptions" :key="option.id" :value="option.id">{{ option.label }}</option>
          </select>
        </div>
