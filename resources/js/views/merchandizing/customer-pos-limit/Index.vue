...
function deleteRow() {
  router.delete(`${props.formMeta.baseUrl}/${confirmingDelete.value.primary_key}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.customer_pos_limit" />

  <BasePageHeading :title="t.customer_pos_limit" :subtitle="t.customer_pos_limit_note">
    <template #extra>
      <button
        v-if="available && can('customer pos limit', 'create')"
  ...
  <div class="content">
    <div v-if="!available" class="alert alert-warning d-flex align-items-center" role="alert">
      ...
      <div>{{ t.legacy_customer_pos_tables_required }}</div>
    </div>

    <BaseBlock :title="t.customer_pos_limit_overview">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-hover table-vcenter fs-sm">
          ...
          <tbody>
            <tr v-if="!rows.length">
              <td colspan="6" class="text-center text-muted py-4">{{ t.no_customer_pos_limits_found }}</td>
            </tr>
            <tr v-for="(record, index) in rows" :key="record.primary_key">
              ...
              <td>{{ record.poslimit }}</td>
              <td>{{ record.posbalance }}</td>
              <td class="text-center text-nowrap">
                <button v-if="can('customer pos limit', 'view')" class="btn btn-sm btn-alt-info me-1" @click="router.get(`${formMeta.baseUrl}/${record.primary_key}`)">
                  <i class="fa fa-eye"></i>
                </button>
                <button v-if="can('customer pos limit', 'edit')" class="btn btn-sm btn-alt-secondary me-1" @click="router.get(`${formMeta.baseUrl}/${record.primary_key}/edit`)">
                  <i class="fa fa-pen"></i>
                </button>
                <button v-if="can('customer pos limit', 'delete')" class="btn btn-sm btn-alt-danger" @click="confirmingDelete = record">
  ...
  <div v-if="confirmingDelete" class="modal fade show d-block" style="background: rgba(0,0,0,.45)">
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">{{ t.delete_customer_pos_limit_confirm.replace(':name', confirmingDelete.customername) }}</div>
