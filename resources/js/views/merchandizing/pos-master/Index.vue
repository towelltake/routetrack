...
const props = defineProps({
  ...
  initialPosData: { type: Object, required: true },
});
...
const form = useForm({
  itemcode: props.initialPosData.itemcode ?? "",
  alternatecode: "",
  itemdescription: "",
  arbitemdescription: "",
  itemvalue: "0.0000",
...
function fillForm(record) {
  ...
  form.itemdescription = record.itemdescription ?? "";
  form.arbitemdescription = record.arbitemdescription ?? "";
  form.itemvalue = record.itemvalue ?? "0.0000";
  form.inventorytype = record.inventorytype ?? 0;
  form.activestatus = record.activestatus ?? 1;
}

function resetForm() {
  form.reset();
  form.itemcode = props.initialPosData.itemcode ?? "";
  form.itemvalue = "0.0000";
  form.inventorytype = 0;
  form.activestatus = 1;
  form.clearErrors();
}
...
function openView(record) {
  editingId.value = record.itemcode;
  form.reset();
  fillForm(record);
  form.clearErrors();
...
function deleteRow() {
  router.delete(`${props.formMeta.baseUrl}/${confirmingDelete.value.itemcode}`, {
    preserveScroll: true,
    onSuccess: () => {
...
function inventoryTypeLabel(value) {
  return Number(value) === 1 ? t.inventory : t.value_label;
}
</script>

<template>
  <Head :title="t.pos_master" />

  <BasePageHeading :title="t.pos_master" :subtitle="t.pos_master_note">
    <template #extra>
      <button
        v-if="available && can('pos master', 'create')"
  ...
  <div class="content">
    <div v-if="!available" class="alert alert-warning d-flex align-items-center" role="alert">
      ...
      <div>{{ t.legacy_pos_master_table_required }}</div>
    </div>

    <BaseBlock :title="t.pos_master_overview">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-hover table-vcenter fs-sm">
          ...
          <tbody>
            <tr v-if="!rows.length">
              <td colspan="7" class="text-center text-muted py-4">{{ t.no_pos_master_records_found }}</td>
            </tr>
            <tr v-for="record in rows" :key="record.itemcode">
              <td class="text-muted">{{ record.itemcode }}</td>
              <td>{{ record.alternatecode || "-" }}</td>
              <td class="fw-semibold">{{ record.itemdescription }}</td>
              <td>{{ record.itemvalue }}</td>
              <td>{{ inventoryTypeLabel(record.inventorytype) }}</td>
              <td>
              ...
              <td class="text-center text-nowrap">
                <button v-if="canViewAction('pos master')" class="btn btn-sm btn-alt-info me-1" @click="openView(record)">
                  <i class="fa fa-eye"></i>
                </button>
                <button v-if="can('pos master', 'edit')" class="btn btn-sm btn-alt-secondary me-1" @click="openEdit(record)">
                  <i class="fa fa-pen"></i>
                </button>
                <button v-if="can('pos master', 'delete')" class="btn btn-sm btn-alt-danger" @click="confirmingDelete = record">
        ...
        <div class="modal-body">
          <div class="row g-4 mb-4">
            ...
            <div class="col-md-4">
              ...
              <select v-model="form.activestatus" class="form-select" :disabled="activeModal === 'view'">
            ...
            <div class="col-md-4">
              ...
              <select v-model="form.inventorytype" class="form-select" :disabled="activeModal === 'view'">
                <option v-for="option in formMeta.inventoryTypeOptions" :key="option.id" :value="option.id">
                  {{ option.label }}
                </option>
              </select>
              <div v-if="form.errors.inventorytype" class="text-danger fs-sm mt-1">{{ form.errors.inventorytype }}</div>
            </div>
          </div>
          ...
          <div class="row g-4">
            ...
            <div class="col-md-6">
              ...
              <input v-model="form.itemvalue" type="number" step="0.0001" min="0" class="form-control" :readonly="activeModal === 'view'" />
              <div v-if="form.errors.itemvalue" class="text-danger fs-sm mt-1">{{ form.errors.itemvalue }}</div>
            </div>
            <div class="col-md-6">
              <label class="form-label">{{ t.description }} <span class="text-danger">*</span></label>
              <input v-model="form.itemdescription" maxlength="50" class="form-control" :readonly="activeModal === 'view'" />
              <div v-if="form.errors.itemdescription" class="text-danger fs-sm mt-1">{{ form.errors.itemdescription }}</div>
            </div>
            <div class="col-md-6">
              <label class="form-label">{{ t.arabic_description }}</label>
              <input v-model="form.arbitemdescription" maxlength="50" class="form-control" dir="rtl" :readonly="activeModal === 'view'" />
              <div v-if="form.errors.arbitemdescription" class="text-danger fs-sm mt-1">{{ form.errors.arbitemdescription }}</div>
            </div>
          </div>
        </div>
  ...
  <div v-if="confirmingDelete" class="modal fade show d-block" style="background: rgba(0,0,0,.45)">
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">{{ t.delete_pos_master_confirm.replace(':name', confirmingDelete.itemdescription) }}</div>
        <div class="modal-footer">
