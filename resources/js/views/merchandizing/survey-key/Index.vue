...
function reloadList(pageNumber = 1) {
  router.get(
    ...
    { preserveScroll: true, preserveState: true, replace: true, only: ["keys", "filters", "available"] },
  );
}

function deleteRow() {
  router.delete(`${props.formMeta.baseUrl}/${confirmingDelete.value.surveykey}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.survey_key" />

  <BasePageHeading :title="t.survey_key" :subtitle="t.survey_key_note">
    <template #extra>
      <button
        v-if="available && can('survey key', 'create')"
  ...
  <div class="content">
    <div v-if="!available" class="alert alert-warning d-flex align-items-center" role="alert">
      ...
      <div>{{ t.legacy_survey_key_tables_required }}</div>
    </div>

    <BaseBlock :title="t.survey_key_overview">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-hover table-vcenter fs-sm">
          ...
          <tbody>
            <tr v-if="!rows.length">
              <td colspan="5" class="text-center text-muted py-4">{{ t.no_survey_keys_found }}</td>
            </tr>
            <tr v-for="(record, index) in rows" :key="record.surveykey">
              <td class="text-muted">{{ (keys.from ?? 1) + index }}</td>
              <td class="fw-semibold">{{ record.surveydescription }}</td>
              <td>{{ record.plan_description || "-" }}</td>
              <td>
              ...
              <td class="text-center text-nowrap">
                <button v-if="can('survey key', 'view')" class="btn btn-sm btn-alt-info me-1" @click="router.get(`${formMeta.baseUrl}/${record.surveykey}`)">
                  <i class="fa fa-eye"></i>
                </button>
                <button v-if="can('survey key', 'edit')" class="btn btn-sm btn-alt-secondary me-1" @click="router.get(`${formMeta.baseUrl}/${record.surveykey}/edit`)">
                  <i class="fa fa-pen"></i>
                </button>
                <button v-if="can('survey key', 'delete')" class="btn btn-sm btn-alt-danger" @click="confirmingDelete = record">
  ...
  <div v-if="confirmingDelete" class="modal fade show d-block" style="background: rgba(0,0,0,.45)">
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">{{ t.delete }} {{ t.survey_key }} <strong>{{ confirmingDelete.surveydescription }}</strong>?</div>
