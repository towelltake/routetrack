...
function reloadList(pageNumber = 1) {
  router.get(
    ...
    { preserveScroll: true, preserveState: true, replace: true, only: ["plans", "filters", "available"] },
  );
}

function deleteRow() {
  router.delete(`${props.formMeta.baseUrl}/${confirmingDelete.value.surveyplankey}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.survey_plan" />

  <BasePageHeading :title="t.survey_plan" :subtitle="t.survey_plan_note">
    <template #extra>
      <button
        v-if="available && can('survey plan', 'create')"
  ...
  <div class="content">
    <div v-if="!available" class="alert alert-warning d-flex align-items-center" role="alert">
      ...
      <div>{{ t.legacy_survey_plan_tables_required }}</div>
    </div>

    <BaseBlock :title="t.survey_plan_overview">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-hover table-vcenter fs-sm">
          ...
          <tbody>
            <tr v-if="!rows.length">
              <td colspan="7" class="text-center text-muted py-4">{{ t.no_survey_plans_found }}</td>
            </tr>
            <tr v-for="(record, index) in rows" :key="record.surveyplankey">
              <td class="text-muted">{{ (plans.from ?? 1) + index }}</td>
              <td class="fw-semibold">{{ record.surveydescription }}</td>
              <td>{{ record.surveysequencenumber }}</td>
              <td>
                <span class="badge" :class="record.surveymandatory ? 'bg-info-subtle text-info' : 'bg-secondary-subtle text-secondary'">
                  {{ record.surveymandatory ? t.mandatory : t.optional }}
                </span>
              </td>
              <td>{{ record.assigned_count }}</td>
              <td>{{ record.remarks || "-" }}</td>
              <td class="text-center text-nowrap">
                <button v-if="can('survey plan', 'view')" class="btn btn-sm btn-alt-info me-1" @click="router.get(`${formMeta.baseUrl}/${record.surveyplankey}`)">
                  <i class="fa fa-eye"></i>
                </button>
                <button v-if="can('survey plan', 'edit')" class="btn btn-sm btn-alt-secondary me-1" @click="router.get(`${formMeta.baseUrl}/${record.surveyplankey}/edit`)">
                  <i class="fa fa-pen"></i>
                </button>
                <button v-if="can('survey plan', 'delete')" class="btn btn-sm btn-alt-danger" @click="confirmingDelete = record">
  ...
  <div v-if="confirmingDelete" class="modal fade show d-block" style="background: rgba(0,0,0,.45)">
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">{{ t.delete }} {{ t.survey_plan }} <strong>{{ confirmingDelete.surveydescription }}</strong>?</div>
