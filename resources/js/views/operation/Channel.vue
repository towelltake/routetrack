...
const filtered = computed(() => {
  ...
  return q ? props.channels.filter(r => r.channelname?.toLowerCase().includes(q) || r.alternatecode?.toLowerCase().includes(q)) : props.channels;
});
...
const form = useForm({ alternatecode: "", channelname: "", arbchannelname: "", activestatus: 1 });

function openAdd() { isEditing.value = false; form.reset(); form.activestatus = 1; form.clearErrors(); showModal.value = true; }
function openEdit(r) {
  isEditing.value = true; editingId.value = r.channelcode;
  form.alternatecode = r.alternatecode ?? "";
  form.channelname = r.channelname;
  form.arbchannelname = r.arbchannelname ?? "";
...
function submit() {
  ...
  isEditing.value ? form.put(`/organisation/channel/${editingId.value}`, opts) : form.post("/organisation/channel", opts);
}
function deleteRow() {
  router.delete(`/organisation/channel/${confirmingDelete.value.channelcode}`, { preserveScroll: true, onSuccess: () => confirmingDelete.value = null });
}
</script>

<template>
  <Head title="Channel Master" />
  <BasePageHeading title="Channel Master" subtitle="Manage customer channels">
    <template #extra>
      <button class="btn btn-primary" @click="openAdd"><i class="fa fa-plus me-1"></i> Add</button>
    </template>
  </BasePageHeading>
  <div class="content">
    <BaseBlock title="Channels">
      <template #options>
        <input v-model="search" type="text" class="form-control form-control-sm" placeholder="Search..." style="width:180px" />
      </template>
      <div class="table-responsive">
        <table class="table table-hover table-vcenter fs-sm">
          <thead><tr><th>#</th><th>Code</th><th>Channel Name</th><th>Status</th><th class="text-center" style="width:100px">Actions</th></tr></thead>
          <tbody>
            <tr v-if="!filtered.length"><td colspan="5" class="text-center text-muted py-4">No records found.</td></tr>
            <tr v-for="(r, i) in filtered" :key="r.channelcode">
              <td class="text-muted">{{ i + 1 }}</td>
              <td>{{ r.alternatecode }}</td>
              <td class="fw-semibold">{{ r.channelname }}</td>
  ...
  <div v-if="showModal" class="modal fade show d-block" style="background:rgba(0,0,0,.45)">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">{{ isEditing ? "Edit Channel" : "Add Channel" }}</h5><button class="btn-close" @click="closeModal"></button></div>
          ...
          <div class="modal-body row g-3">
            ...
            <div class="col-6 d-flex align-items-end">
              <div class="form-check">
                <input v-model="form.activestatus" :true-value="1" :false-value="0" type="checkbox" class="form-check-input" id="channelactive" />
            ...
            <div class="col-6">
              ...
              <input v-model="form.channelname" class="form-control" :class="{'is-invalid':form.errors.channelname}" maxlength="50" />
              <div class="invalid-feedback">{{ form.errors.channelname }}</div>
            </div>
            <div class="col-6">
              <label class="form-label">Arabic Name</label>
              <input v-model="form.arbchannelname" class="form-control" dir="rtl" maxlength="50" />
            </div>
          </div>
  ...
  <div v-if="confirmingDelete" class="modal fade show d-block" style="background:rgba(0,0,0,.45)">
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">Delete <strong>{{ confirmingDelete.channelname }}</strong>?</div>
        <div class="modal-footer">
