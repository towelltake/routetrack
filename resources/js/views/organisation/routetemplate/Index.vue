...
function deleteRow() {
  router.delete(`/organisation/routetemplate/${confirmingDelete.value.routecode}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.route_template_setting" />

  <BasePageHeading :title="t.route_template_setting" :subtitle="t.route_setting_template_note">
    <template #extra>
      <button
        v-if="can('route template', 'create')"
        class="btn btn-primary"
        @click="router.get('/organisation/routetemplate/create')"
      >
        <i class="fa fa-plus me-1"></i> {{ t.add_route_setting_template }}
      </button>
    </template>
  </BasePageHeading>

  <div class="content">
    <BaseBlock :title="t.route_setting_template_list">
      <template #options>
        <div class="d-flex gap-2">
          <input v-model="search" type="text" class="form-control form-control-sm" :placeholder="`${t.search}...`" style="width: 200px" />
        ...
        <table class="table table-hover table-vcenter fs-sm">
          <thead>
            <tr>
              <th>#</th>
              <th>{{ t.code }}</th>
              <th>{{ t.route_setting_template_name }}</th>
              <th>{{ t.route_category }}</th>
              <th>{{ t.status }}</th>
          ...
          <tbody>
            ...
            <tr v-for="(record, index) in rows" :key="record.routecode">
              ...
              <td class="text-center text-nowrap">
                <button
                  v-if="canViewAction('route template')"
                  class="btn btn-sm btn-alt-info me-1"
                  :title="t.view"
                  @click="router.get(`/organisation/routetemplate/${record.routecode}`)"
                >
                  <i class="fa fa-eye"></i>
                </button>
                <button
                  v-if="can('route template', 'edit')"
                  class="btn btn-sm btn-alt-secondary me-1"
                  :title="t.edit"
                  @click="router.get(`/organisation/routetemplate/${record.routecode}/edit`)"
                >
                  <i class="fa fa-pen"></i>
                </button>
                <button
                  v-if="can('route template', 'delete')"
                  class="btn btn-sm btn-alt-danger"
                  :title="t.delete_route_setting_template_label"
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title text-danger"><i class="fa fa-triangle-exclamation me-1"></i> {{ t.delete_route_setting_template_label }}</h5>
          <button class="btn-close" @click="confirmingDelete = null"></button>
        </div>
        <div class="modal-body">{{ t.route_setting_template_delete_confirm }}</div>
        <div class="modal-footer">
          <button class="btn btn-alt-secondary" @click="confirmingDelete = null">{{ t.cancel }}</button>
          <button class="btn btn-danger" @click="deleteRow">{{ t.delete_route_setting_template_label }}</button>
        </div>
      </div>
    </div>
  </div>
