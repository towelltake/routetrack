...
const accountsReportGroup = computed(() => {
  if (!reportsModule || !canView(reportsModule.permission)) return null;
  const group = reportsModule.groups.find((item) => item.key === "accounts-reports");
  if (!group) return null;
  return { ...group, items: group.items.filter((item) => canView(item.permission)) };
});
...
<template>
  <Head :title="t.accounts_report" />
  <BasePageHeading :title="t.accounts_report" :subtitle="t.accounts_report_note" />
  <div class="content">
    <BaseBlock :title="t.available_reports">
      <div class="row g-4">
        <div v-for="report in accountsReportGroup?.items ?? []" :key="report.key" class="col-md-6">
          <BaseBlock tag="button" class="w-100 h-100 text-start" transparent @click="router.get(report.href)">
            <div class="d-flex align-items-start gap-3">
              <div class="fs-2 text-primary"><i :class="report.icon"></i></div>
              <div>
