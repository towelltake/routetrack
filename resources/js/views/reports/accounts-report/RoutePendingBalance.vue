...
const columnCount = computed(() => props.periodColumns.length + 3);
...
function reload(page = 1) {
  router.get("/reports/accounts-report/route-pending-balance", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
    only: ["filters", "sort", "rows", "totals", "pagination", "filterScopeRows", "companyOptions", "regionOptions", "depotOptions", "areaOptions", "subAreaOptions", "routeOptions", "yearOptions", "periodColumns"],
  });
}
...
function sort(column) {
  if (sortBy.value === column) sortDir.value = sortDir.value === "asc" ? "desc" : "asc";
    ...
    sortBy.value = column;
    sortDir.value = ["route_label", "salesman_label"].includes(column) ? "asc" : "desc";
  }
  reload();
}
...
function exportUrl(type) {
  ...
  return `/reports/accounts-report/route-pending-balance/export/${type}?${params.toString()}`;
}
...
<template>
  <Head :title="t.route_pending_balance" />
  <BasePageHeading :title="t.route_pending_balance" :subtitle="t.route_pending_balance_report_note" />
  <div class="content">
    <BaseBlock :title="t.global_report_filters">
      <div class="row g-3 align-items-end mb-3">
        ...
        <div class="col-md-3"><label class="form-label">{{ t.year }}</label><VueSelect v-model="yearValue" class="report-filter-select" :options="yearOptions" label="label" :placeholder="t.select_year" :clearable="true" /></div>
    ...
    <BaseBlock :title="t.route_pending_balance">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              <th class="cursor-pointer text-nowrap" @click="sort('route_label')">{{ t.route }} <i class="fa ms-1" :class="sortIcon('route_label')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('salesman_label')">{{ t.salesman }} <i class="fa ms-1" :class="sortIcon('salesman_label')"></i></th>
              ...
              <th class="cursor-pointer text-end text-nowrap" @click="sort('total')">{{ t.total }} <i class="fa ms-1" :class="sortIcon('total')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount" class="text-center text-muted py-4">{{ t.no_records_found }}</td></tr>
            <tr v-for="row in rows" :key="`${row.routecode_sort}-${row.salesmancode_sort}`">
          ...
          <tfoot>
            <tr>
              <td colspan="2" class="text-end fw-semibold">{{ t.total }}</td>
