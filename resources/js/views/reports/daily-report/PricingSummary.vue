...
function reload(page = 1) {
  router.get("/reports/daily-report/pricing-summary", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
...
function exportUrl(type) {
  ...
  return `/reports/daily-report/pricing-summary/export/${type}?${params.toString()}`;
}
...
function toggleDetails(key) {
  expandedInvoices.value = {
    ...expandedInvoices.value,
    [key]: !expandedInvoices.value[key],
  };
}

function isExpanded(key) {
  return !!expandedInvoices.value[key];
}
</script>

<template>
  <Head :title="t.pricing_summary" />

  <BasePageHeading
    :title="t.pricing_summary"
    :subtitle="t.pricing_summary_report_note"
  />
  ...
  <div class="content">
    <BaseBlock title="Global Report Filters">
      <div class="row g-3 align-items-end mb-3">
        ...
        <div class="col-md-2"><label class="form-label">From Date</label><input v-model="transactionDateFrom" type="date" class="form-control" /></div>
        <div class="col-md-2"><label class="form-label">To Date</label><input v-model="transactionDateTo" type="date" class="form-control" /></div>
    ...
    <BaseBlock title="Pricing Summary Report">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              <th class="text-center" style="width: 48px;">Details</th>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('reportcustcode')">Customer Code <i class="fa ms-1" :class="sortIcon('reportcustcode')"></i></th>
              ...
              <th class="cursor-pointer text-nowrap text-end" @click="sort('totalinvoiceamount')">Sales Amount <i class="fa ms-1" :class="sortIcon('totalinvoiceamount')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('totalfreesampleamount')">Free Amount <i class="fa ms-1" :class="sortIcon('totalfreesampleamount')"></i></th>
          ...
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount + 1" class="text-center text-muted py-4">No records found.</td></tr>
            <template v-for="(row, index) in rows" :key="`${row.transactionkey}-${row.invoicenumber}-${index}`">
              <tr>
                <td class="text-center">
                  <button type="button" class="btn btn-sm btn-alt-secondary" @click="toggleDetails(row.transactionkey)">
                    <i class="fa" :class="isExpanded(row.transactionkey) ? 'fa-chevron-up' : 'fa-chevron-down'"></i>
                  </button>
                </td>
                ...
                <td class="text-end">{{ formatAmount(row.totalfreesampleamount) }}</td>
                <td class="text-end">{{ formatAmount(row.netamount) }}</td>
              </tr>
              <tr v-if="isExpanded(row.transactionkey)">
                <td :colspan="columnCount + 1" class="p-0">
                  <div class="table-responsive bg-body-light">
                    <table class="table table-sm table-bordered table-striped mb-0 detail-table">
                      <thead>
                        <tr>
                          <th>Item Code</th>
                          <th>Item Description</th>
                          <th class="text-end">Sales Qty</th>
                          <th class="text-end">Sales Case Price</th>
                          <th class="text-end">Sales Pcs Price</th>
                          <th class="text-end">Std. Case Price</th>
                          <th class="text-end">Std. Pcs Price</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr v-if="!row.details?.length">
                          <td colspan="7" class="text-center text-muted">No pricing detail found.</td>
                        </tr>
                        <tr v-for="(detail, detailIndex) in row.details ?? []" :key="`${row.transactionkey}-${detail.itemcode}-${detailIndex}`">
                          <td>{{ detail.itemcode || "-" }}</td>
                          <td>{{ detail.itemdescription || "-" }}</td>
                          <td class="text-end">{{ formatAmount(detail.salesqty) }}</td>
                          <td class="text-end">{{ formatAmount(detail.salescaseprice) }}</td>
                          <td class="text-end">{{ formatAmount(detail.salesprice) }}</td>
                          <td class="text-end">{{ formatAmount(detail.stdsalescaseprice) }}</td>
                          <td class="text-end">{{ formatAmount(detail.stdsalesprice) }}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
          ...
          <tfoot v-if="rows.length">
            <tr class="table-light fw-semibold">
              ...
              <td class="text-end">{{ formatAmount(totals.totalinvoiceamount) }}</td>
              <td class="text-end">{{ formatAmount(totals.totalfreesampleamount) }}</td>
