...
function reload(page = 1) {
  router.get("/reports/daily-report/route-activity", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
...
function exportUrl(type) {
  ...
  return `/reports/daily-report/route-activity/export/${type}?${params.toString()}`;
}
...
<template>
  <Head :title="t.route_activity" />

  <BasePageHeading
    :title="t.route_activity"
    :subtitle="t.route_activity_report_note"
  />
  ...
  <div class="content">
    <BaseBlock :title="t.global_report_filters">
      <div class="row g-3 align-items-end mb-3">
        <div class="col-md-3">
          ...
          <VueSelect v-model="companyValue" class="report-filter-select" :options="filteredCompanyOptions" label="label" :placeholder="t.all_companies" :clearable="true" />
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t.region }}</label>
          <VueSelect v-model="regionValue" class="report-filter-select" :options="filteredRegionOptions" label="label" :placeholder="t.all_regions" :clearable="true" />
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t.branch_depot }}</label>
          <VueSelect v-model="depotValue" class="report-filter-select" :options="filteredDepotOptions" label="label" :placeholder="t.all_branches_depots" :clearable="true" />
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t.area }}</label>
          <VueSelect v-model="areaValue" class="report-filter-select" :options="filteredAreaOptions" label="label" :placeholder="t.all_areas" :clearable="true" />
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t.sub_area }}</label>
          <VueSelect v-model="subAreaValue" class="report-filter-select" :options="filteredSubAreaOptions" label="label" :placeholder="t.all_sub_areas" :clearable="true" />
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t.route }}</label>
          <VueSelect v-model="routeValue" class="report-filter-select" :options="filteredRouteOptions" label="label" :placeholder="t.all_routes" :clearable="true" />
        </div>
        <div class="col-md-3">
    ...
    <BaseBlock :title="t.route_activity">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              <th class="cursor-pointer text-nowrap" @click="sort('trandate')">{{ t.transaction_date }} <i class="fa ms-1" :class="sortIcon('trandate')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('routecode')">{{ t.route }} <i class="fa ms-1" :class="sortIcon('routecode')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('customercode')">{{ t.customer }} <i class="fa ms-1" :class="sortIcon('customercode')"></i></th>
              <th class="text-nowrap">{{ t.address }}</th>
              <th class="text-nowrap">{{ t.standard_gps }}</th>
              <th class="text-nowrap">{{ t.actual_gps }}</th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('distance')">{{ t.difference_meter }} <i class="fa ms-1" :class="sortIcon('distance')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('visitstarttime')">{{ t.visit_start_time }} <i class="fa ms-1" :class="sortIcon('visitstarttime')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('visitendtime')">{{ t.visit_end_time }} <i class="fa ms-1" :class="sortIcon('visitendtime')"></i></th>
              <th class="text-nowrap">{{ t.duration_hhmm }}</th>
              <th class="text-nowrap">{{ t.visit_interval_time }}</th>
              <th class="cursor-pointer text-nowrap" @click="sort('trantype')">{{ t.transaction }} <i class="fa ms-1" :class="sortIcon('trantype')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('tranamount')">{{ t.order_collected }} <i class="fa ms-1" :class="sortIcon('tranamount')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('receiptpaid')">{{ t.receipt_collected }} <i class="fa ms-1" :class="sortIcon('receiptpaid')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length">
            ...
            <tr v-for="(row, index) in rows" :key="`${row.routekey}-${row.visitkey}-${row.documentno}-${index}`">
              <td>{{ row.trandate || "-" }}</td>
              ...
              <td>{{ row.trantype || "-" }}</td>
              <td class="text-end">{{ formatAmount(row.tranamount) }}</td>
              <td class="text-end">{{ formatAmount(row.receiptpaid) }}</td>
            </tr>
          </tbody>
          <tfoot v-if="rows.length">
            <tr class="table-light fw-semibold">
              <td colspan="12" class="text-end">{{ t.total }}</td>
              <td class="text-end">{{ formatAmount(totals.tranamount) }}</td>
              <td class="text-end">{{ formatAmount(totals.receiptpaid) }}</td>
            </tr>
          </tfoot>
        </table>
      </div>
