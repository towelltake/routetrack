...
const sortBy = ref(props.sort.by ?? "routeenddate_sort");
...
function reload(page = 1) {
  router.get("/reports/daily-report/route-deposit-summary", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
...
function sort(column) {
  if (sortBy.value === column) {
    ...
    sortBy.value = column;
    sortDir.value = column === "routeenddate_sort" ? "asc" : "desc";
  }
  reload();
}
...
function resetFilters() {
  ...
  routeCode.value = "";
  perPage.value = 25;
  sortBy.value = "routeenddate_sort";
  sortDir.value = "asc";
  reload();
}
...
function exportUrl(type) {
  ...
  return `/reports/daily-report/route-deposit-summary/export/${type}?${params.toString()}`;
}
...
<template>
  <Head :title="t.route_deposit_summary" />

  <BasePageHeading
    :title="t.route_deposit_summary"
    :subtitle="t.route_deposit_summary_report_note"
  />
  ...
  <div class="content">
    <BaseBlock :title="t.global_report_filters">
      <div class="row g-3 align-items-end mb-3">
        ...
        <div class="col-md-2"><label class="form-label">{{ t.transaction_date_from }}</label><input v-model="routeEndDateFrom" type="date" class="form-control" /></div>
        <div class="col-md-2"><label class="form-label">{{ t.transaction_date_to }}</label><input v-model="routeEndDateTo" type="date" class="form-control" /></div>
    ...
    <BaseBlock :title="t.route_deposit_summary">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('routecode')">{{ t.route_code }} <i class="fa ms-1" :class="sortIcon('routecode')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('type')">{{ t.invoice_type }} <i class="fa ms-1" :class="sortIcon('type')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('paytype')">{{ t.payment_type }} <i class="fa ms-1" :class="sortIcon('paytype')"></i></th>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('againstinv_sort')">{{ t.against_invoice }} <i class="fa ms-1" :class="sortIcon('againstinv_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('saleinvoiceddate_sort')">{{ t.invoiced_date }} <i class="fa ms-1" :class="sortIcon('saleinvoiceddate_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('salesmancode')">{{ t.invoiced_salesman }} <i class="fa ms-1" :class="sortIcon('salesmancode')"></i></th>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('bankname_sort')">{{ t.bank_name }} <i class="fa ms-1" :class="sortIcon('bankname_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('checknumber_sort')">{{ t.check_number }} <i class="fa ms-1" :class="sortIcon('checknumber_sort')"></i></th>
              <th class="text-nowrap">{{ t.draw_date }}</th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('amount')">{{ t.amount }} <i class="fa ms-1" :class="sortIcon('amount')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount" class="text-center text-muted py-4">{{ t.no_records_found }}</td></tr>
            <tr v-for="(row, index) in rows" :key="`${row.type}-${row.routekey}-${row.visitkey}-${row.invoicenumber}-${index}`">
              ...
              <td>{{ row.againstinv || "-" }}</td>
              <td>{{ row.saleinvoiceddate || "-" }}</td>
          ...
          <tfoot v-if="rows.length">
            <tr class="table-light fw-semibold">
              <td colspan="14" class="text-end">{{ t.total }}</td>
              <td class="text-end">{{ formatAmount(totals.amount) }}</td>
            </tr>
          </tfoot>
        </table>
      </div>
