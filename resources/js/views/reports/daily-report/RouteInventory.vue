...
function reload(page = 1) {
  router.get("/reports/daily-report/route-inventory", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
...
function exportUrl(type) {
  ...
  return `/reports/daily-report/route-inventory/export/${type}?${params.toString()}`;
}
...
<template>
  <Head :title="t.route_inventory" />

  <BasePageHeading
    :title="t.route_inventory"
    :subtitle="t.route_inventory_report_note"
  />
  ...
  <div class="content">
    ...
    <BaseBlock :title="t.route_inventory">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('majorcategorycode')">{{ t.group }} <i class="fa ms-1" :class="sortIcon('majorcategorycode')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('actualitemcode')">{{ t.item_code }} <i class="fa ms-1" :class="sortIcon('actualitemcode')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('itemdescription')">{{ t.item_description }} <i class="fa ms-1" :class="sortIcon('itemdescription')"></i></th>
              <th class="text-nowrap">{{ t.opening_case_pcs }}</th>
              <th class="text-nowrap">{{ t.load_case_pcs }}</th>
              <th class="text-nowrap">{{ t.transfer_in_case_pcs }}</th>
              <th class="text-nowrap">{{ t.transfer_out_case_pcs }}</th>
              <th class="text-nowrap">{{ t.sales_case_pcs }}</th>
              <th class="text-nowrap">{{ t.good_return_case_pcs }}</th>
              <th class="text-nowrap">{{ t.bad_return_case_pcs }}</th>
              <th class="text-nowrap">{{ t.free_case_pcs }}</th>
              <th class="text-nowrap">{{ t.damage_variance_qty_pcs }}</th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('damagevariancevalue')">{{ t.damage_variance_value }} <i class="fa ms-1" :class="sortIcon('damagevariancevalue')"></i></th>
              <th class="text-nowrap">{{ t.closing_case_pcs }}</th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('truckstockvalue')">{{ t.load_value }} <i class="fa ms-1" :class="sortIcon('truckstockvalue')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('endstockvalue')">{{ t.closing_value }} <i class="fa ms-1" :class="sortIcon('endstockvalue')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount" class="text-center text-muted py-4">{{ t.no_records_found }}</td></tr>
            <tr v-for="(row, index) in rows" :key="`${row.routekey}-${row.actualitemcode}-${index}`">
          ...
          <tfoot v-if="rows.length">
            <tr class="table-light fw-semibold">
              <td colspan="13" class="text-end">{{ t.total }}</td>
