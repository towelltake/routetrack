...
function reload(page = 1) {
  router.get("/reports/daily-report/route-trip-analysis", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
...
function exportUrl(type) {
  ...
  return `/reports/daily-report/route-trip-analysis/export/${type}?${params.toString()}`;
}
...
<template>
  <Head :title="t.route_trip_analysis" />

  <BasePageHeading
    :title="t.route_trip_analysis"
    :subtitle="t.route_trip_analysis_report_note"
  />
  ...
  <div class="content">
    <BaseBlock :title="t.global_report_filters">
      <div class="row g-3 align-items-end mb-3">
        ...
        <div class="col-md-3"><label class="form-label">{{ t.route_end_date }}</label><input v-model="routeEndDate" type="date" class="form-control" /></div>
    ...
    <BaseBlock :title="t.route_trip_analysis">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('dayname')">{{ t.week_day }} <i class="fa ms-1" :class="sortIcon('dayname')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('visitsequence')">{{ t.visit_sequence }} <i class="fa ms-1" :class="sortIcon('visitsequence')"></i></th>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('customername_sort')">{{ t.customer }} <i class="fa ms-1" :class="sortIcon('customername_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('mop')">{{ t.payment_mode }} <i class="fa ms-1" :class="sortIcon('mop')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('visitsheduled')">{{ t.customer_visit_schedule }} <i class="fa ms-1" :class="sortIcon('visitsheduled')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('visitstarttime_sort')">{{ t.visit_start_time }} <i class="fa ms-1" :class="sortIcon('visitstarttime_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('visitendtime_sort')">{{ t.visit_end_time }} <i class="fa ms-1" :class="sortIcon('visitendtime_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('visitstatus')">{{ t.visit_status }} <i class="fa ms-1" :class="sortIcon('visitstatus')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('totalinvoiceamount')">{{ t.transaction_amount }} <i class="fa ms-1" :class="sortIcon('totalinvoiceamount')"></i></th>
              <th class="text-nowrap">{{ t.no_transaction_reason }}</th>
              <th class="cursor-pointer text-nowrap" @click="sort('lastsales_sort')">{{ t.last_invoiced_date }} <i class="fa ms-1" :class="sortIcon('lastsales_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('lastorder_sort')">{{ t.last_order_taken_on }} <i class="fa ms-1" :class="sortIcon('lastorder_sort')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount" class="text-center text-muted py-4">{{ t.no_records_found }}</td></tr>
            <tr v-for="(row, index) in rows" :key="`${row.routekey}-${row.visitkey}-${row.trantype}-${index}`">
              <td class="fw-semibold">{{ row.route_label || "-" }}</td>
              <td>{{ row.dayname || "-" }}</td>
              <td>{{ row.visitsequence || "-" }}</td>
              <td>{{ row.reportcustcode || "-" }}</td>
              <td>{{ row.customer_label || "-" }}</td>
              <td>{{ row.mop || "-" }}</td>
              <td>{{ row.visitsheduled || "-" }}</td>
          ...
          <tfoot v-if="rows.length">
            <tr class="table-light fw-semibold">
              <td colspan="10" class="text-end">{{ t.total }}</td>
