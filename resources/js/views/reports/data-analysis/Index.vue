...
const dataAnalysisGroup = computed(() => {
  if (!reportsModule || !canView(reportsModule.permission)) return null;
  const group = reportsModule.groups.find((item) => item.key === "data-analysis");
...
<template>
  <Head :title="t.data_analysis" />
  <BasePageHeading :title="t.data_analysis" :subtitle="t.data_analysis_note" />
  <div class="content">
    <BaseBlock :title="t.available_reports">
      <div class="row g-4">
        <div v-for="report in dataAnalysisGroup?.items ?? []" :key="report.key" class="col-md-6">
