...
function reload(page = 1) {
  router.get("/reports/data-analysis/item-group-wise-sales", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
    only: ["filters", "sort", "rows", "totals", "pagination", "filterScopeRows", "companyOptions", "regionOptions", "depotOptions", "areaOptions", "subAreaOptions", "routeOptions", "majorCategoryOptions"],
  });
}
...
function sort(column) {
  if (sortBy.value === column) sortDir.value = sortDir.value === "asc" ? "desc" : "asc";
    ...
    sortBy.value = column;
    sortDir.value = ["transactiondate_sort", "routecode_sort", "itemgroupcode_sort", "itemcode_sort"].includes(column) ? "asc" : "desc";
  }
  reload();
}
...
function exportUrl(type) {
  ...
  return `/reports/data-analysis/item-group-wise-sales/export/${type}?${params.toString()}`;
}
...
<template>
  <Head title="Item Group Wise Sales" />
  <BasePageHeading title="Item Group Wise Sales" subtitle="Review route-wise sales, returns, free goods, and discounts by item group." />
  <div class="content">
    ...
    <BaseBlock title="Item Group Wise Sales Report">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('itemgroupcode_sort')">Item Category <i class="fa ms-1" :class="sortIcon('itemgroupcode_sort')"></i></th>
              ...
              <th class="cursor-pointer text-end text-nowrap" @click="sort('returnqty')">G. Return Qty <i class="fa ms-1" :class="sortIcon('returnqty')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('good_return_inv_price')">G. Return @ Inv. Price <i class="fa ms-1" :class="sortIcon('good_return_inv_price')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('damagedqty')">Damaged Return Qty <i class="fa ms-1" :class="sortIcon('damagedqty')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('damage_inv_price')">Damaged Return @ Inv. Price <i class="fa ms-1" :class="sortIcon('damage_inv_price')"></i></th>
              ...
              <th class="cursor-pointer text-end text-nowrap" @click="sort('expired_inv_price')">Expired @ Inv. Price <i class="fa ms-1" :class="sortIcon('expired_inv_price')"></i></th>
              ...
              <th class="cursor-pointer text-end text-nowrap" @click="sort('discounts')">Discounts <i class="fa ms-1" :class="sortIcon('discounts')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('total_amount')">Total Amount <i class="fa ms-1" :class="sortIcon('total_amount')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount" class="text-center text-muted py-4">No records found.</td></tr>
            <tr v-for="row in rows" :key="`${row.transactiondate_sort}-${row.routecode_sort}-${row.itemgroupcode_sort}-${row.itemcode_sort}`">
              ...
              <td class="text-end">{{ formatAmount(row.discounts) }}</td>
          ...
          <tfoot>
            <tr>
              ...
              <td class="text-end fw-semibold">{{ formatAmount(totals.discounts) }}</td>
              <td class="text-end fw-semibold">{{ formatAmount(totals.total_amount) }}</td>
            </tr>
          </tfoot>
        </table>
      </div>
