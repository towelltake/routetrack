...
const sortBy = ref(props.sort.by ?? "majorcategorycode_sort");
...
function reload(page = 1) {
  router.get("/reports/data-analysis/item-sales-summary", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
...
function sort(column) {
  if (sortBy.value === column) sortDir.value = sortDir.value === "asc" ? "desc" : "asc";
    ...
    sortBy.value = column;
    sortDir.value = ["transactiondate_sort", "majorcategorycode_sort", "itemgroupcode_sort", "itemcode_sort"].includes(column) ? "asc" : "desc";
  }
  reload();
}
...
function resetFilters() {
  ...
  perPage.value = 25;
  sortBy.value = "majorcategorycode_sort";
  sortDir.value = "asc";
  reload();
}
...
function exportUrl(type) {
  ...
  return `/reports/data-analysis/item-sales-summary/export/${type}?${params.toString()}`;
}
...
<template>
  <Head title="Item Sales Summary" />
  <BasePageHeading title="Item Sales Summary" subtitle="Review item-wise sales, returns, expiry, discounts, and free goods." />
  <div class="content">
    ...
    <BaseBlock title="Item Sales Summary Report">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('itemgroupcode_sort')">Item Group Code <i class="fa ms-1" :class="sortIcon('itemgroupcode_sort')"></i></th>
              ...
              <th class="cursor-pointer text-end text-nowrap" @click="sort('damagedqty')">Damage Ret. Qty <i class="fa ms-1" :class="sortIcon('damagedqty')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('expiryqty')">Expired Qty <i class="fa ms-1" :class="sortIcon('expiryqty')"></i></th>
              ...
              <th class="cursor-pointer text-end text-nowrap" @click="sort('damage_std_price')">Damage Ret @ Std. Price <i class="fa ms-1" :class="sortIcon('damage_std_price')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('damage_inv_price')">Damage Ret @ Inv. Price <i class="fa ms-1" :class="sortIcon('damage_inv_price')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('expired_std_price')">Expired Ret @ Std. Price <i class="fa ms-1" :class="sortIcon('expired_std_price')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('expired_inv_price')">Expired Ret @ Inv. Price <i class="fa ms-1" :class="sortIcon('expired_inv_price')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('good_return_std_price')">Good Return @ Std. Price <i class="fa ms-1" :class="sortIcon('good_return_std_price')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('good_return_inv_price')">Good Return @ Inv. Price <i class="fa ms-1" :class="sortIcon('good_return_inv_price')"></i></th>
              ...
              <th class="cursor-pointer text-end text-nowrap" @click="sort('net_sales')">Net Sales <i class="fa ms-1" :class="sortIcon('net_sales')"></i></th>
          ...
          <tbody>
            ...
            <tr v-for="row in rows" :key="`${row.transactiondate_sort}-${row.majorcategorycode_sort}-${row.itemgroupcode_sort}-${row.itemcode_sort}`">
              ...
              <td class="text-end">{{ formatAmount(row.expiryqty) }}</td>
              ...
              <td class="text-end">{{ formatAmount(row.damage_std_price) }}</td>
              <td class="text-end">{{ formatAmount(row.damage_inv_price) }}</td>
              <td class="text-end">{{ formatAmount(row.expired_std_price) }}</td>
              <td class="text-end">{{ formatAmount(row.expired_inv_price) }}</td>
              <td class="text-end">{{ formatAmount(row.good_return_std_price) }}</td>
              <td class="text-end">{{ formatAmount(row.good_return_inv_price) }}</td>
          ...
          <tfoot>
            <tr>
              ...
              <td class="text-end fw-semibold">{{ formatAmount(totals.expiryqty) }}</td>
              ...
              <td class="text-end fw-semibold">{{ formatAmount(totals.damage_std_price) }}</td>
              <td class="text-end fw-semibold">{{ formatAmount(totals.damage_inv_price) }}</td>
              <td class="text-end fw-semibold">{{ formatAmount(totals.expired_std_price) }}</td>
              <td class="text-end fw-semibold">{{ formatAmount(totals.expired_inv_price) }}</td>
              <td class="text-end fw-semibold">{{ formatAmount(totals.good_return_std_price) }}</td>
              <td class="text-end fw-semibold">{{ formatAmount(totals.good_return_inv_price) }}</td>
              ...
              <td class="text-end fw-semibold">{{ formatAmount(totals.net_sales) }}</td>
