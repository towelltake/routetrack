...
function reload(page = 1) {
  router.get("/reports/data-analysis/route-monthly-revenue", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
...
function resetFilters() {
  ...
  subAreaCode.value = "";
  routeCode.value = "";
  year.value = props.yearOptions[0]?.id ? String(props.yearOptions[0].id) : "";
  perPage.value = 25;
...
function exportUrl(type) {
  ...
  return `/reports/data-analysis/route-monthly-revenue/export/${type}?${params.toString()}`;
}
...
<template>
  <Head title="Route Monthly Revenue" />
  <BasePageHeading title="Route Monthly Revenue" subtitle="Review route revenue by month for the selected year." />
  <div class="content">
    <BaseBlock title="Global Report Filters">
      <div class="row g-3 align-items-end mb-3">
        ...
        <div class="col-md-3"><label class="form-label">Year</label><VueSelect v-model="yearValue" class="report-filter-select" :options="yearOptions" label="label" placeholder="Select Year" :clearable="true" /></div>
    ...
    <BaseBlock title="Route Monthly Revenue Report">
      <template #options>
