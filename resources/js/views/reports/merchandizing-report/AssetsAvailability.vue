...
function reload(page = 1) {
  router.get("/reports/merchandizing-report/assets-availability", currentParams(page), { preserveScroll: true, preserveState: true, replace: true, only: ["filters", "sort", "rows", "pagination", "filterScopeRows", "companyOptions", "regionOptions", "depotOptions", "areaOptions", "subAreaOptions", "routeOptions", "customerOptions"] });
}
function sort(column) { if (sortBy.value === column) sortDir.value = sortDir.value === "asc" ? "desc" : "asc"; else { sortBy.value = column; sortDir.value = ["route_label", "customer_code", "customer_name", "pos_code", "pos_description", "serial_number", "pos_status", "remarks"].includes(column) ? "asc" : "desc"; } reload(); }
...
function exportUrl(type) { const params = new URLSearchParams(); Object.entries(currentParams()).forEach(([key, value]) => { if (value !== undefined && value !== null && value !== "") params.set(key, String(value)); }); return `/reports/merchandizing-report/assets-availability/export/${type}?${params.toString()}`; }
</script>

<template>
  <Head title="Assets Availability" />
  <BasePageHeading title="Assets Availability" subtitle="Review customer asset records captured during visits, including POS code, serial, status, and remarks." />
  <div class="content">
    ...
    <BaseBlock title="Assets Availability Report">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('pos_code')">POS Code <i class="fa ms-1" :class="sortIcon('pos_code')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('pos_description')">POS <i class="fa ms-1" :class="sortIcon('pos_description')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('serial_number')">Serial No <i class="fa ms-1" :class="sortIcon('serial_number')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('pos_status')">POS Status <i class="fa ms-1" :class="sortIcon('pos_status')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('remarks')">Remarks <i class="fa ms-1" :class="sortIcon('remarks')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount" class="text-center text-muted py-4">No records found.</td></tr>
            <tr v-for="(row, index) in rows" :key="`${row.routekey}-${row.visitkey}-${row.pos_code}-${row.serial_number}-${index}`">
