...
const merchandizingReportGroup = computed(() => {
  if (!reportsModule || !canView(reportsModule.permission)) {
    return null;
  }

  const group = reportsModule.groups.find((item) => item.key === "merchandizing-reports");
...
<template>
  <Head :title="t.merchandizing_report" />

  <BasePageHeading
    :title="t.merchandizing_report"
    :subtitle="t.merchandizing_report_note"
  />
  ...
  <div class="content">
    <BaseBlock :title="t.available_reports">
      <div class="row g-4">
        <div
          v-for="report in merchandizingReportGroup?.items ?? []"
          :key="report.key"
          class="col-md-6"
        >
          <BaseBlock
