...
function reload(page = 1) {
  router.get("/reports/merchandizing-report/merchandized-stock", currentParams(page), { preserveScroll: true, preserveState: true, replace: true, only: ["filters", "sort", "rows", "pagination", "filterScopeRows", "companyOptions", "regionOptions", "depotOptions", "areaOptions", "subAreaOptions", "routeOptions", "customerOptions"] });
}
function sort(column) { if (sortBy.value === column) sortDir.value = sortDir.value === "asc" ? "desc" : "asc"; else { sortBy.value = column; sortDir.value = ["route_label", "customer_code", "customer_name", "item_code", "item_description", "shelf_stock", "store_stock"].includes(column) ? "asc" : "desc"; } reload(); }
...
function exportUrl(type) { const params = new URLSearchParams(); Object.entries(currentParams()).forEach(([key, value]) => { if (value !== undefined && value !== null && value !== "") params.set(key, String(value)); }); return `/reports/merchandizing-report/merchandized-stock/export/${type}?${params.toString()}`; }
...
<template>
  <Head title="Merchandized Stock" />
  <BasePageHeading title="Merchandized Stock" subtitle="Review shelf and store stock checks captured during visits, including cut-off and max quantities." />
  <div class="content">
    ...
    <BaseBlock title="Merchandized Stock Report">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-end text-nowrap" @click="sort('cutoff_qty')">Cut-off Qty <i class="fa ms-1" :class="sortIcon('cutoff_qty')"></i></th>
              <th class="cursor-pointer text-end text-nowrap" @click="sort('max_qty')">Max Qty <i class="fa ms-1" :class="sortIcon('max_qty')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('shelf_stock')">Shelf <i class="fa ms-1" :class="sortIcon('shelf_stock')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('store_stock')">Store <i class="fa ms-1" :class="sortIcon('store_stock')"></i></th>
