...
function reload(page = 1) {
  router.get("/reports/merchandizing-report/survey-tracking", currentParams(page), { preserveScroll: true, preserveState: true, replace: true, only: ["filters", "sort", "rows", "pagination", "filterScopeRows", "companyOptions", "regionOptions", "depotOptions", "areaOptions", "subAreaOptions", "routeOptions"] });
}
function sort(column) { if (sortBy.value === column) sortDir.value = sortDir.value === "asc" ? "desc" : "asc"; else { sortBy.value = column; sortDir.value = ["route_label", "salesman_label", "customer_code", "customer_name", "survey_description", "survey_response"].includes(column) ? "asc" : "desc"; } reload(); }
...
function exportUrl(type) { const params = new URLSearchParams(); Object.entries(currentParams()).forEach(([key, value]) => { if (value !== undefined && value !== null && value !== "") params.set(key, String(value)); }); return `/reports/merchandizing-report/survey-tracking/export/${type}?${params.toString()}`; }
</script>

<template>
  <Head title="Survey Tracking" />
  <BasePageHeading title="Survey Tracking" subtitle="Review customer survey answers captured during route visits, grouped by route, salesman, and outlet." />
  <div class="content">
    ...
    <BaseBlock title="Survey Tracking Report">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('survey_description')">Survey Description <i class="fa ms-1" :class="sortIcon('survey_description')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('survey_response')">Survey Response <i class="fa ms-1" :class="sortIcon('survey_response')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount" class="text-center text-muted py-4">No records found.</td></tr>
            <tr v-for="(row, index) in rows" :key="`${row.routekey}-${row.visitkey}-${index}`">
