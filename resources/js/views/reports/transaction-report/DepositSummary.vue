...
function reload(page = 1) {
  router.get("/reports/transaction-report/deposit-summary", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
...
function exportUrl(type) {
  ...
  return `/reports/transaction-report/deposit-summary/export/${type}?${params.toString()}`;
}
...
<template>
  <Head title="Deposit Summary" />

  <BasePageHeading
    title="Deposit Summary"
    subtitle="Review invoice and receipt deposits by transaction date, route, payment type, and total amount."
  />
  ...
  <div class="content">
    ...
    <BaseBlock title="Deposit Summary Report">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('customer_sort')">Customer <i class="fa ms-1" :class="sortIcon('customer_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('documentnumber_sort')">Receipt <i class="fa ms-1" :class="sortIcon('documentnumber_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('type_sort')">Type <i class="fa ms-1" :class="sortIcon('type_sort')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('immediatecash')">Immediate Cash <i class="fa ms-1" :class="sortIcon('immediatecash')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('immediatecheck')">Immediate Cheque <i class="fa ms-1" :class="sortIcon('immediatecheck')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('total')">Total <i class="fa ms-1" :class="sortIcon('total')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount" class="text-center text-muted py-4">No records found.</td></tr>
            <tr v-for="(row, index) in rows" :key="`${row.type}-${row.routekey}-${row.visitkey}-${row.documentnumber}-${index}`">
          ...
          <tfoot v-if="rows.length">
            <tr class="table-light fw-semibold">
              ...
              <td class="text-end">{{ formatAmount(totals.total) }}</td>
            </tr>
          </tfoot>
        </table>
      </div>
