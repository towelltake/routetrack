...
function reload(page = 1) {
  router.get("/reports/transaction-report/final-deposit", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
...
function exportUrl(type) {
  ...
  return `/reports/transaction-report/final-deposit/export/${type}?${params.toString()}`;
}
...
<template>
  <Head title="Final Deposit" />

  <BasePageHeading
    title="Final Deposit"
    subtitle="Review final deposited collection amounts by receipt, customer, invoice reference, and payment method."
  />
  ...
  <div class="content">
    ...
    <BaseBlock title="Final Deposit Report">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('documentnumber_sort')">Receipt Number <i class="fa ms-1" :class="sortIcon('documentnumber_sort')"></i></th>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('customercode_sort')">Customer <i class="fa ms-1" :class="sortIcon('customercode_sort')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('arcash')">Amount Paid in CASH <i class="fa ms-1" :class="sortIcon('arcash')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('archeck')">Amount Paid in CHEQUE <i class="fa ms-1" :class="sortIcon('archeck')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('totalinvoiceamount')">Total Amount <i class="fa ms-1" :class="sortIcon('totalinvoiceamount')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('alternateinvoicenumber_sort')">Against Invoice <i class="fa ms-1" :class="sortIcon('alternateinvoicenumber_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('invoicedby_sort')">Invoiced By <i class="fa ms-1" :class="sortIcon('invoicedby_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('invoicedate_sort')">Invoiced Date <i class="fa ms-1" :class="sortIcon('invoicedate_sort')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('amountpaid')">Paid Amount <i class="fa ms-1" :class="sortIcon('amountpaid')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount" class="text-center text-muted py-4">No records found.</td></tr>
            <tr v-for="(row, index) in rows" :key="`${row.transactionkey}-${row.documentnumber}-${index}`">
              ...
              <td>{{ row.alternateinvoicenumber || "-" }}</td>
              <td>{{ row.invoicedby || "-" }}</td>
              <td>{{ row.invoicedate || "-" }}</td>
              <td class="text-end">{{ formatAmount(row.amountpaid) }}</td>
            </tr>
          </tbody>
