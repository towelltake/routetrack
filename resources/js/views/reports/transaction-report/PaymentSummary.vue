...
const paginationLabel = computed(() => {
  if (!props.rows.length) return "No records found";
  if (props.pagination.total == null) return `Showing ${props.pagination.from ?? 0} to ${props.pagination.to ?? 0}`;
...
function reload(page = 1) {
  router.get("/reports/transaction-report/payment-summary", currentParams(page), {
    preserveScroll: true,
    preserveState: true,
    replace: true,
...
function exportUrl(type) {
  ...
  return `/reports/transaction-report/payment-summary/export/${type}?${params.toString()}`;
}
...
<template>
  ...
  <BasePageHeading
    ...
    subtitle="Review invoice-wise payments, collections, and outstanding balances for the selected period."
  />
  ...
  <div class="content">
    ...
    <BaseBlock title="Payment Summary Report">
      <template #options>
        ...
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('type_sort')">Type <i class="fa ms-1" :class="sortIcon('type_sort')"></i></th>
              ...
              <th class="cursor-pointer text-nowrap" @click="sort('mop')">MOP <i class="fa ms-1" :class="sortIcon('mop')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('totalinvoiceamount')">Total Invoice Amount <i class="fa ms-1" :class="sortIcon('totalinvoiceamount')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('immediatecash')">Immediate CASH Payment <i class="fa ms-1" :class="sortIcon('immediatecash')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('immediatecheck')">Immediate Cheque Payment <i class="fa ms-1" :class="sortIcon('immediatecheck')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('arcash')">AR CASH Collection <i class="fa ms-1" :class="sortIcon('arcash')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('archeck')">AR Cheque Collection <i class="fa ms-1" :class="sortIcon('archeck')"></i></th>
              <th class="cursor-pointer text-nowrap text-end" @click="sort('outstandingbalanceamount')">Outstanding Balance Amount <i class="fa ms-1" :class="sortIcon('outstandingbalanceamount')"></i></th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr v-for="(row, index) in rows" :key="`${row.routekey}-${row.visitkey}-${row.invoicenumber}-${index}`">
              ...
              <td class="text-end">{{ formatAmount(row.immediatecash) }}</td>
              <td class="text-end">{{ formatAmount(row.immediatecheck) }}</td>
              <td class="text-end">{{ formatAmount(row.arcash) }}</td>
              <td class="text-end">{{ formatAmount(row.archeck) }}</td>
              <td class="text-end">{{ formatAmount(row.outstandingbalanceamount) }}</td>
            </tr>
          </tbody>
          <tfoot v-if="rows.length">
            <tr class="table-light fw-semibold">
              ...
              <td class="text-end">{{ formatAmount(totals.immediatecash) }}</td>
              <td class="text-end">{{ formatAmount(totals.immediatecheck) }}</td>
              <td class="text-end">{{ formatAmount(totals.arcash) }}</td>
              <td class="text-end">{{ formatAmount(totals.archeck) }}</td>
              <td class="text-end">{{ formatAmount(totals.outstandingbalanceamount) }}</td>
            </tr>
          </tfoot>
        </table>
      </div>
      ...
      <div class="d-flex justify-content-between align-items-center pt-3">
        ...
        <div class="btn-group">
          ...
          <button class="btn btn-sm btn-alt-secondary" :disabled="!pagination.has_more_pages" @click="reload(pagination.current_page + 1)">Next</button>
        </div>
      </div>
    </BaseBlock>
  </div>
