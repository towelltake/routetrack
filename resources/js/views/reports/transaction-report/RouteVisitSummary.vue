...
function reload(page = 1) {
  router.get("/reports/transaction-report/route-visit-summary", currentParams(page), { preserveScroll: true, preserveState: true, replace: true, only: ["filters", "sort", "rows", "totals", "pagination", "filterScopeRows", "companyOptions", "regionOptions", "depotOptions", "areaOptions", "subAreaOptions", "routeOptions"] });
}
function sort(column) { if (sortBy.value === column) sortDir.value = sortDir.value === "asc" ? "desc" : "asc"; else { sortBy.value = column; sortDir.value = column === "routecode" ? "asc" : "desc"; } reload(); }
...
function resetFilters() { const today = new Date().toISOString().slice(0, 10); transactionDateFrom.value = today; transactionDateTo.value = today; companyCode.value = ""; regionCode.value = ""; depotCode.value = ""; areaCode.value = ""; subAreaCode.value = ""; routeCode.value = ""; perPage.value = 25; sortBy.value = "routecode"; sortDir.value = "asc"; reload(); }
function exportUrl(type) { const params = new URLSearchParams(); Object.entries(currentParams()).forEach(([key, value]) => { if (value !== undefined && value !== null && value !== "") params.set(key, String(value)); }); return `/reports/transaction-report/route-visit-summary/export/${type}?${params.toString()}`; }
</script>

<template>
  <Head :title="t.route_visit_summary" />
  <BasePageHeading :title="t.route_visit_summary" :subtitle="t.route_visit_summary_report_note" />
  <div class="content">
    ...
    <BaseBlock :title="t.route_visit_summary">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-vcenter fs-sm">
          <thead>
            <tr>
              <th class="cursor-pointer text-nowrap" @click="sort('routecode')">{{ t.route }} <i class="fa ms-1" :class="sortIcon('routecode')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('routestartdate_sort')">{{ t.route_start_date }} <i class="fa ms-1" :class="sortIcon('routestartdate_sort')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('daytxt')">{{ t.day }} <i class="fa ms-1" :class="sortIcon('daytxt')"></i></th>
              <th class="cursor-pointer text-nowrap" @click="sort('routeenddate_sort')">{{ t.route_end_date }} <i class="fa ms-1" :class="sortIcon('routeenddate_sort')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('targettovisit')">{{ t.scheduled_targets_to_visit }} <i class="fa ms-1" :class="sortIcon('targettovisit')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('targetvisits')">{{ t.scheduled_targets_visited }} <i class="fa ms-1" :class="sortIcon('targetvisits')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('callexceptions')">{{ t.scheduled_targets_not_visited }} <i class="fa ms-1" :class="sortIcon('callexceptions')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('nontargetvisits')">{{ t.unscheduled_targets_visited }} <i class="fa ms-1" :class="sortIcon('nontargetvisits')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('totalvisits')">{{ t.total_visits }} <i class="fa ms-1" :class="sortIcon('totalvisits')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('schedulesale')">{{ t.schedule_sale }} <i class="fa ms-1" :class="sortIcon('schedulesale')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('schedulenosale')">{{ t.schedule_no_sale }} <i class="fa ms-1" :class="sortIcon('schedulenosale')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('unschedsale')">{{ t.unscheduled_sale }} <i class="fa ms-1" :class="sortIcon('unschedsale')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('unschedulenosale')">{{ t.unscheduled_no_sale }} <i class="fa ms-1" :class="sortIcon('unschedulenosale')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('effectivevisit')">{{ t.effective_visit }} <i class="fa ms-1" :class="sortIcon('effectivevisit')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('startkms')">{{ t.starting_kms }} <i class="fa ms-1" :class="sortIcon('startkms')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('endkms')">{{ t.ending_kms }} <i class="fa ms-1" :class="sortIcon('endkms')"></i></th>
              <th class="text-end cursor-pointer" @click="sort('kmscovered')">{{ t.total_kms_covered }} <i class="fa ms-1" :class="sortIcon('kmscovered')"></i></th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!rows.length"><td :colspan="columnCount" class="text-center text-muted py-4">{{ t.no_records_found }}</td></tr>
            <tr v-for="(row, index) in rows" :key="`${row.routekey}-${row.routeenddate_sort}-${index}`">
