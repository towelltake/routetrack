        ...
        <section v-if="view === 'performance'" ref="performanceSection" class="analytics-table-panel" aria-labelledby="performance-title">
            ...
            <div class="analytics-table-scroll" tabindex="0" aria-label="Scrollable performance comparison">
                <table><thead><tr>
                    ...
                    <th scope="col">Customers</th><th scope="col"><button @click="sortBy('operational_minutes')">Operational min ↕</button></th><th scope="col">CFT variance</th>
                    ...
                    <tr v-for="group in visibleGroups" :key="group.id">
                        ...
                        <td>{{ percent(group.productive, group.completed) }}<small>{{ group.productive }} / {{ group.completed }} visits</small><small>Collection: {{ percent(group.collection_productive, group.completed) }}</small><small>Sales orders + invoices: {{ percent(group.sales_order_productive, group.completed) }}</small></td>
                        <td>{{ number(group.customers.size) }}</td><td>{{ number(group.operational_minutes, 1) }}</td><td>{{ variance(group) }}</td>
