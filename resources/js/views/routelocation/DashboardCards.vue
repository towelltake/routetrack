...
const cards = computed(() => {
    ...
    return [
        ...
        { title: "Customer Face Time", icon: "fa-clock", tone: "violet", value: number(m?.cft_minutes, 1), unit: "min",
            note: m ? (variance == null ? "Variance unavailable · No configured CFT" : `${variance > 0 ? "+" : ""}${number(variance, 1)} min vs expected · ${number(m.cft_configured_visits)} visits`) : "",
            definition: "Actual duration of completed visits. Variance includes only visits with a positive expected CFT, using customer CFT then channel CFT." },
