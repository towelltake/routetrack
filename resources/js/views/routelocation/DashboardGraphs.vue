...
const cft = computed(() => series(cftRoutes.value, [['Planned (min)', 'expected_cft', '#c4b5fd'], ['Operational (min)', 'configured_actual_cft', '#7c3aed']]));
const timeRoutes = computed(() => [...routes.value].filter(row => row.duration_count).sort((a, b) => b.duration - a.duration).slice(0, 10));
const time = computed(() => series(timeRoutes.value, [['Operational time (min)', 'visit_time', '#14b8a6'], ['Time outside visits (min)', 'remaining_time', '#cbd5e1']]));
...
<template>
    <section class="dashboard-graphs" aria-label="Performance graphs">
        ...
        <DashboardChart title="Planned vs operational time" description="Top 10 routes with recorded planned CFT" :data="cft" :options="horizontal" :loading="loading" note="Compares the same visits on both sides." />
        <DashboardChart class="time-graph" title="Where journey time goes" description="Top 10 routes by measured journey duration" :data="time" :options="timeOptions" :loading="loading" note="Open journeys use the last reported location. Operational time sums completed visits. Time outside visits equals duration minus operational time." />
    </section>
</template>
