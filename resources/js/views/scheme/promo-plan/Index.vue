...
function deleteRow() {
  router.delete(`/scheme/promotion/promo-plan/${confirmingDelete.value.plannumber}`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Head :title="t.promo_plan" />

  <BasePageHeading :title="t.promo_plan" :subtitle="t.promo_plan_note">
    <template #extra>
      <button
        v-if="available && can('promo plan', 'create')"
        class="btn btn-primary"
        @click="router.get('/scheme/promotion/promo-plan/create')"
      >
        <i class="fa fa-plus me-1"></i> {{ t.create_promo_plan }}
      </button>
    </template>
  </BasePageHeading>
  ...
  <div class="content">
    <div v-if="!available" class="alert alert-warning d-flex align-items-center" role="alert">
      ...
      <div>
        Legacy tables <code>promoplandetail</code> and <code>promoplanheader</code> are required for
        the promo plan workflow.
      </div>
    </div>

    <BaseBlock :title="t.promo_plan_overview">
      <template #options>
      ...
      <div class="table-responsive">
        <table class="table table-hover table-vcenter fs-sm">
          ...
          <tbody>
            <tr v-if="!rows.length">
              <td colspan="6" class="text-center text-muted py-4">{{ t.no_promo_plans_found }}</td>
            </tr>
            <tr v-for="(record, index) in rows" :key="record.plannumber">
              <td class="text-muted">{{ (plans.from ?? 1) + index }}</td>
              <td>{{ record.plannumber }}</td>
              <td>{{ record.plandescription || "-" }}</td>
              <td>{{ record.arbplandescription || "-" }}</td>
              <td>
                <span
                  class="badge"
              ...
              <td class="text-center text-nowrap">
                <button
                  v-if="canViewAction('promo plan')"
                  class="btn btn-sm btn-alt-info me-1"
                  :title="t.view"
                  @click="router.get(`/scheme/promotion/promo-plan/${record.plannumber}`)"
                >
                  <i class="fa fa-eye"></i>
                </button>
                <button
                  v-if="can('promo plan', 'edit')"
                  class="btn btn-sm btn-alt-secondary me-1"
                  :title="t.edit"
                  @click="router.get(`/scheme/promotion/promo-plan/${record.plannumber}/edit`)"
                >
                  <i class="fa fa-pen"></i>
                </button>
                <button
                  v-if="can('promo plan', 'delete')"
      ...
      <div class="d-flex justify-content-between align-items-center pt-3">
        <div class="text-muted fs-sm">
          {{ t.showing }} {{ plans.from ?? 0 }} {{ t.to }} {{ plans.to ?? 0 }} {{ t.of }} {{ plans.total ?? 0 }}
        </div>
        <div class="btn-group">
  ...
  <div v-if="confirmingDelete" class="modal fade show d-block" style="background: rgba(0,0,0,.45)">
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">{{ t.delete }} <strong>{{ confirmingDelete.plandescription }}</strong>?</div>
