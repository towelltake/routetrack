...
function reloadList(pageNumber = 1) {
  router.get("/transaction/advance-payment", {
...
function detailUrl(transactionkey) {
  ...
  return `/transaction/advance-payment/${transactionkey}?${params.toString()}`;
}
...
<template>
  ...
  <div class="content">
    <BaseBlock :title="t.advance_payment_list ?? 'Advance Payment List'">
      <template #options>
