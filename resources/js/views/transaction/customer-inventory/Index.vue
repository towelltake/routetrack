...
const sortBy = ref(props.filters?.sort_by ?? "visitdate");
...
function reloadList(pageNumber = 1) {
  router.get("/transaction/customer-inventory", {
...
function detailUrl(primaryId) {
  ...
  return `/transaction/customer-inventory/${primaryId}?${params.toString()}`;
}
</script>
...
<template>
  ...
  <div class="content">
    <BaseBlock :title="t.customer_inventory_list ?? 'Customer Inventory List'">
      <template #options>
        ...
        <table class="table table-hover table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer" @click="sort('visitdate')">
                {{ t.visit_date ?? "Visit Date" }}<i class="fa ms-1" :class="sortIcon('visitdate')"></i>
              </th>
              ...
              <th class="cursor-pointer" @click="sort('alternatecode')">
                {{ t.alternate_code ?? "Alternate Code" }}<i class="fa ms-1" :class="sortIcon('alternatecode')"></i>
              </th>
          ...
          <tbody>
            ...
            <tr v-for="record in rows" :key="record.primary_id">
              ...
              <td class="text-center">
                <Link class="btn btn-sm btn-alt-info" :href="detailUrl(record.primary_id)">
                  <i class="fa fa-eye"></i>
                </Link>
              </td>
            </tr>
