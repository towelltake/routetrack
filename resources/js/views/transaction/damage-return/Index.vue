...
function reloadList(pageNumber = 1) {
  router.get("/transaction/damage-return", {
...
function detailUrl(transactionkey) {
  ...
  return `/transaction/damage-return/${transactionkey}?${params.toString()}`;
}
...
<template>
  ...
  <div class="content">
    <BaseBlock :title="t.damage_return_list ?? 'Damage Return List'">
      <template #options>
        ...
        <table class="table table-hover table-vcenter fs-sm">
          <thead>
            <tr>
              ...
              <th class="cursor-pointer text-end" @click="sort('transactiontime')">
          ...
          <tbody>
            ...
            <tr v-for="record in rows" :key="record.transactionkey">
              ...
              <td class="text-end">{{ record.transactiontime || "-" }}</td>
              <td class="text-end">{{ amount(record.totaldamagedamount) }}</td>
