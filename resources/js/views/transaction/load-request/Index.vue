...
function reloadList(pageNumber = 1) {
  router.get("/transaction/load-request", {
...
function detailUrl(detailkey) {
  ...
  return `/transaction/load-request/${detailkey}?${params.toString()}`;
}
</script>

<template>
  <Head :title="t.load_request ?? 'Load Request'" />

  <BasePageHeading
    :title="t.load_request ?? 'Load Request'"
    :subtitle="t.load_request_note ?? 'Review route-wise load requests using the legacy overview workflow'"
  >
    <template #extra>
      <Link v-if="can('load request', 'create')" href="/transaction/load-request/create" class="btn btn-primary">
        <i class="fa fa-plus me-1"></i> {{ t.create_load_request ?? "Create Load Request" }}
      </Link>
    </template>
  </BasePageHeading>

  <div class="content">
    <BaseBlock :title="t.load_request_list ?? 'Load Request List'">
      <template #options>
