...
const props = defineProps({
  ...
  lineTypeOptions: { type: Array, required: true },
  selectedLineType: { type: Number, required: true },
...
const backUrl = computed(() => {
  ...
  return `/transaction/load-transfer?${params.toString()}`;
});
...
function transferRouteLabel() {
  return locale === "ar"
    ? (props.header.arbtransferroutename || props.header.transferroutename || "")
    : (props.header.transferroutename || props.header.arbtransferroutename || "");
}
...
function updateLineType(event) {
  ...
  if (props.filters?.sort_by) params.set("sort_by", props.filters.sort_by);
  if (props.filters?.sort_dir) params.set("sort_dir", props.filters.sort_dir);
  params.set("line_type", String(Number(event.target.value || 3)));

  router.get(`/transaction/load-transfer/${props.header.detailkey}?${params.toString()}`, {}, {
    preserveScroll: true,
    preserveState: true,
    replace: true,
  });
  ...
  <div class="content">
    <BaseBlock :title="t.details ?? 'Details'">
      <div class="row g-3 fs-sm mb-4">
        ...
        <div class="col-md-6">
          <label class="form-label">{{ t.transfer_route ?? "Transfer Route" }}</label>
          <div class="form-control-plaintext">
            <template v-if="header.transferlocationcode">
              {{ header.transferlocationcode }} - {{ transferRouteLabel() || "-" }}
            </template>
            <template v-else>-</template>
          </div>
        </div>
        <div class="col-md-6">
          <label class="form-label">{{ t.load_no ?? "Load No." }}</label>
          <div class="form-control-plaintext">{{ header.loadnumber || "-" }}</div>
        </div>
      </div>

      <div class="row g-3 align-items-end mb-3">
        <div class="col-md-4">
          <label class="form-label">{{ t.transaction_type ?? "Transaction Type" }}</label>
          <select class="form-select" :value="selectedLineType" @change="updateLineType">
        ...
        <table class="table table-hover table-vcenter fs-sm">
          ...
          <tfoot v-if="lines.length">
            <tr class="fw-semibold">
              <td colspan="5" class="text-end">{{ t.total ?? "Total" }}</td>
