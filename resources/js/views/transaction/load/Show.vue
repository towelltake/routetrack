...
const backUrl = computed(() => {
  ...
  return `/transaction/load?${params.toString()}`;
});
...
<template>
  <Head :title="t.load ?? 'Load'" />

  <BasePageHeading
    :title="t.load ?? 'Load'"
    :subtitle="t.load_note ?? 'Review route-wise load transactions using the legacy overview workflow'"
  >
    <template #extra>
