...
const backUrl = computed(() => {
  ...
  return `/transaction/sales-order?${params.toString()}`;
});
...
const totals = computed(() => props.lines.reduce((carry, line) => ({
  ...
  taxorder: carry.taxorder + Number(line.taxorder || 0),
...
function deliveryRouteLabel() {
  return locale === "ar"
    ? (props.header.arbdeliveryroutename || props.header.deliveryroutename || "")
    : (props.header.deliveryroutename || props.header.arbdeliveryroutename || "");
}
...
const printUrl = computed(() => `/transaction/sales-order/${props.header.transactionkey}/print`);
</script>
  ...
  <div class="content">
    <BaseBlock :title="t.details ?? 'Details'">
      <div class="row g-3 fs-sm mb-4">
        ...
        <div class="col-md-3">
          <label class="form-label">{{ t.invoice_no ?? "Order No." }}</label>
          <div class="form-control-plaintext">{{ header.invoicenumber || "-" }}</div>
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t.transaction_date ?? "Order Date" }}</label>
          <div class="form-control-plaintext">{{ header.transactiondate || "-" }}</div>
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t.transaction_time ?? "Order Time" }}</label>
        ...
        <div class="col-md-4">
          <label class="form-label">{{ t.payment_term ?? "Customer Payment Term" }}</label>
        ...
        <div class="col-md-3">
          <label class="form-label">{{ t.document_valid ?? "Order Status" }}</label>
        ...
        <div class="col-md-4">
          <label class="form-label">{{ t.delivery_route ?? "Delivery Route" }}</label>
          <div class="form-control-plaintext">
            <template v-if="header.orderdeliveryroutecode">
              {{ header.orderdeliveryroutecode }} - {{ deliveryRouteLabel() || "-" }}
            </template>
            <template v-else>-</template>
          </div>
        </div>
        <div class="col-md-4">
          <label class="form-label">{{ t.delivery_date ?? "Delivery Date" }}</label>
          <div class="form-control-plaintext">{{ header.orderdeliverydate || "-" }}</div>
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
          ...
          <div class="form-control-plaintext">{{ header.comments2 || "-" }}</div>
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-3">
          <label class="form-label">Final Order Amount</label>
          <div class="form-control-plaintext">{{ amount(header.totalinvoiceamount) }}</div>
        </div>
        <div class="col-md-3">
          <label class="form-label">Total Returned Amount</label>
          <div class="form-control-plaintext">{{ amount(header.totalreturnamount) }}</div>
        </div>
        <div class="col-md-3">
          <label class="form-label">Total Damaged Amount</label>
          <div class="form-control-plaintext">{{ amount(header.totaldamagedamount) }}</div>
        </div>
        <div class="col-md-3">
          <label class="form-label">Total Free Amount</label>
          <div class="form-control-plaintext">{{ amount(header.totalfreesampleamount) }}</div>
        </div>
        <div class="col-md-3">
          <label class="form-label">Total Line Item Discount</label>
          <div class="form-control-plaintext">{{ amount(header.lineitemdiscount) }}</div>
        </div>
        <div class="col-md-3">
          <label class="form-label">Total Order Discount</label>
          <div class="form-control-plaintext">{{ amount(header.orderdiscount) }}</div>
        </div>
        <div class="col-md-3">
          <label class="form-label">Total Tax Amount</label>
          <div class="form-control-plaintext">{{ amount(header.totallineitemtax) }}</div>
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-3">
          ...
          <div class="form-control-plaintext">{{ amount(header.totalexcisetax) }}</div>
        </div>
        <div v-if="header.signaturedata" class="col-12">
          <label class="form-label">Signature</label>
          <div>
            <img
              :src="header.signaturedata"
              alt="Signature"
      ...
      <div class="table-responsive">
        <table class="table table-hover table-vcenter fs-sm">
          <thead>
            <tr>
              <th rowspan="2">{{ t.alt_code ?? "Alternate Code" }}</th>
              <th rowspan="2">{{ t.item_name ?? "Item Description" }}</th>
              <th rowspan="2" class="text-center">{{ t.upc ?? "UPC" }}</th>
              <th colspan="4" class="text-center">Order</th>
            ...
            <tr>
              ...
              <th class="text-end">Case Price</th>
          ...
          <tbody>
            ...
            <tr v-for="line in lines" :key="line.primary_key">
              ...
              <td class="text-end">{{ amount(line.taxorder) }}</td>
          ...
          <tfoot v-if="lines.length">
            <tr class="fw-semibold">
              ...
              <td class="text-end">{{ amount(totals.taxorder) }}</td>
