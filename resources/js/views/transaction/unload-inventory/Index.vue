...
function reloadList(pageNumber = 1) {
  router.get("/transaction/unload-inventory", {
...
function detailUrl(detailkey) {
  ...
  return `/transaction/unload-inventory/${detailkey}?${params.toString()}`;
}
</script>

<template>
  <Head :title="t.unload_inventory ?? 'Unload Inventory'" />

  <BasePageHeading
    :title="t.unload_inventory ?? 'Unload Inventory'"
    :subtitle="t.unload_inventory_note ?? 'Review unload inventory records by transaction date.'"
  />

  <div class="content">
    <BaseBlock :title="t.unload_inventory_list ?? 'Unload Inventory List'">
      <template #options>
