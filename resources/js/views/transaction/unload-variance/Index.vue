...
function reloadList(pageNumber = 1) {
  router.get("/transaction/unload-variance", {
...
function detailUrl(detailkey) {
  ...
  return `/transaction/unload-variance/${detailkey}?${params.toString()}`;
}
</script>

<template>
  <Head :title="t.unload_variance ?? 'Unload Variance'" />

  <BasePageHeading
    :title="t.unload_variance ?? 'Unload Variance'"
    :subtitle="t.unload_variance_note ?? 'Review unload variance records by transaction date.'"
  />

  <div class="content">
    <BaseBlock :title="t.unload_variance_list ?? 'Unload Variance List'">
      <template #options>
